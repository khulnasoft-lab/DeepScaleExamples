...
SAMPLE_SP = os.path.join(os.path.dirname(os.path.abspath(__file__)),
...
class MarianTokenizationTest(TokenizerTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        save_json(mock_tokenizer_config,
    ...
    def test_tokenizer_equivalence_en_de(self):
        ...
        batch = en_de_tokenizer.prepare_seq2seq_batch(["I am a small frog"],
                                                      return_tensors=None)
    ...
    def test_outputs_can_be_shorter(self):
        tok = self.get_tokenizer()
        batch_smaller = tok.prepare_seq2seq_batch(
