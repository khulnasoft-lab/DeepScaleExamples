...
class LEDModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        inputs_dict = prepare_led_inputs_dict(config, input_ids,
                                              decoder_input_ids)
    ...
    def check_global_attention(self, config, inputs_dict):
        ...
        attention_mask = ids_tensor(inputs_dict["input_ids"].shape,
                                    vocab_size=2)
...
class LEDModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = ((LEDForConditionalGeneration, )
                                    if is_torch_available() else ())
    ...
    def _check_encoder_attention_for_generate(self, attentions, batch_size,
        ...
        tgt_length = (seq_length // config.attention_window[0] +
    ...
    def _check_encoder_hidden_states_for_generate(self, hidden_states,
        ...
        seq_length = (seq_length // config.attention_window[0] +
                      (seq_length % config.attention_window[0] != 0)
                      ) * config.attention_window[0]
...
class LEDModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        inputs_dict = prepare_led_inputs_dict(model.config, input_ids,
                                              decoder_input_ids)
        with torch.no_grad():
