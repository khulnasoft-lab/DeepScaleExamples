...
class ModelTesterMixin:
    ...
    def test_save_load__keys_to_ignore_on_save(self):
        ...
        for model_class in self.all_model_classes:
            model = model_class(config)
            _keys_to_ignore_on_save = getattr(model, "_keys_to_ignore_on_save",
                                              None)
        ...
        for model_class in self.all_model_classes:
            ...
            def check_attentions_validity(attentions):
                ...
                if (
                    ...
                    self.assertNotEqual(
                        attentions[1][..., 0, :, :].flatten().sum().item(),
                        0.0)
                self.assertAlmostEqual(
    ...
    def test_hidden_states_output(self):
        def check_hidden_states_output(inputs_dict, config, model_class):
            ...
            if hasattr(self.model_tester, "encoder_seq_length"):
                ...
                if (hasattr(self.model_tester, "chunk_length")
        ...
        for model_class in self.all_model_classes:
            ...
            if hasattr(model, base_model_prefix):
                with tempfile.TemporaryDirectory() as temp_dir_name:
                    ...
                    with self.subTest(msg="Missing keys for {}".format(
    ...
    def test_model_parallelization(self):
        ...
        for model_class in self.all_parallelizable_model_classes:
            ...
            for n in range(torch.cuda.device_count()):
                self.assertGreater(memory_after_parallelization[n],
                                   memory_at_start[n])

            # Assert that the memory use of device 0 is lower than it was when the entire model was loaded on it
            self.assertLess(memory_after_parallelization[0],
                            memory_after_model_load[0])
            ...
            self.assertGreater(memory_after_parallelization[1],
                               memory_after_model_load[1])
        ...
        for model_class in self.all_parallelizable_model_classes:
            ...
            for value, parallel_value in zip(output, parallel_output):
                if isinstance(value, torch.Tensor):
                    self.assertTrue(
                        torch.allclose(value,
                                       parallel_value.to("cpu"),
                                       atol=1e-7))
                elif isinstance(value, (Tuple, List)):
                    for value_, parallel_value_ in zip(value, parallel_value):
                        self.assertTrue(
                            torch.allclose(value_,
                                           parallel_value_.to("cpu"),
                                           atol=1e-7))
    ...
    def test_model_parallel_beam_search(self):
        ...
        for model_class in all_generative_and_parallelizable_model_classes:
            ...
            model.parallelize()
            model.generate(**cast_to_device(inputs_dict, "cuda:0"),
                           num_beams=2)
...
def ids_tensor(shape, vocab_size, rng=None, name=None):
    ...
    return (torch.tensor(data=values, dtype=torch.long,
                         device=torch_device).view(shape).contiguous())
