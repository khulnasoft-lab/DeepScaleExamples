...
class GenerationTesterMixin:
    ...
    def _get_encoder_outputs(
        ...
        encoder_outputs["last_hidden_state"] = (
            encoder_outputs.last_hidden_state.repeat_interleave(num_interleave,
                                                                dim=0))
        input_ids = (torch.zeros_like(input_ids[:, :1]) +
    ...
    def _beam_sample_generate(
        ...
        with torch.no_grad():
            output_beam_sample = model.beam_sample(
                input_ids.repeat_interleave(beam_scorer.num_beams *
                                            num_return_sequences,
                                            dim=0),
                beam_scorer,
                max_length=max_length,
    ...
    def test_greedy_generate_dict_outputs(self):
        for model_class in self.all_generative_model_classes:
            ...
            if model.config.is_encoder_decoder:
                self.assertIsInstance(output_greedy,
                                      GreedySearchEncoderDecoderOutput)
                self.assertIsInstance(output_generate,
                                      GreedySearchEncoderDecoderOutput)
            else:
                self.assertIsInstance(output_greedy,
                                      GreedySearchDecoderOnlyOutput)
    ...
    def test_sample_generate_dict_output(self):
        for model_class in self.all_generative_model_classes:
            ...
            if model.config.is_encoder_decoder:
                self.assertIsInstance(output_sample,
                                      SampleEncoderDecoderOutput)
                self.assertIsInstance(output_generate,
                                      SampleEncoderDecoderOutput)
            else:
    ...
    def _check_outputs(self,
        ...
        if config.is_encoder_decoder:
            ...
            attentions = output.attentions if not use_cache else output.attentions[
                1:]
        ...
        if config.is_encoder_decoder:
            ...
            hidden_states = (output.hidden_states
                             if not use_cache else output.hidden_states[1:])
            min_length = seq_length if not use_cache else seq_length + 1
            self._check_hidden_states_for_generate(
                num_sequences_in_output,
                hidden_states,
    ...
    def _check_encoder_attention_for_generate(self, attentions, batch_size,
                                              config, seq_length):
    ...
    def _check_encoder_hidden_states_for_generate(self, hidden_states,
                                                  batch_size, config,
                                                  seq_length):
...
class UtilsFunctionsTest(unittest.TestCase):
    ...
    def test_top_k_top_p_filtering(self):
        ...
        non_inf_output = output[output != -float("inf")].to(
            device=torch_device)
        non_inf_idx = (output != -float("inf")).nonzero().to(
            device=torch_device)

        self.assertTrue(
            torch.allclose(non_inf_expected_output, non_inf_output,
                           atol=1e-12))
