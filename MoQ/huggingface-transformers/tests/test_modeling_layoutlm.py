...
def prepare_layoutlm_batch_inputs():
    ...
    labels = torch.tensor(
        [[
            -100, 10, 10, 10, 9, 1, -100, 7, 7, -100, 7, 7, 4, 2, 5, 2, 8, 8,
            -100, -100, 5, 0, 3, 2, -100
        ],
         [
             -100, 12, 12, 12, -100, 12, 10, -100, -100, -100, -100, 10, 12, 9,
             -100, -100, -100, 10, 10, 10, 9, 12, -100, 10, -100
         ]],
        device=torch_device)  # noqa: E231
    # fmt: on
...
class LayoutLMModelIntegrationTest(unittest.TestCase):
    ...
    def test_forward_pass_no_head(self):
        ...
        expected_slice = torch.tensor([-0.6580, -0.0214, 0.8552],
                                      device=torch_device)

        self.assertTrue(
            torch.allclose(outputs.pooler_output[1, :3],
                           expected_slice,
                           atol=1e-3))
    ...
    def test_forward_pass_sequence_classification(self):
        ...
        model = LayoutLMForSequenceClassification.from_pretrained(
            "microsoft/layoutlm-base-uncased", num_labels=2).to(torch_device)
