...
class RagTestMixin:
    ...
    def get_retriever(self, config):
        ...
        tokenizer = (self.bart_tokenizer if config.generator.model_type
                     == "bart" else self.t5_tokenizer)
...
class RagModelIntegrationTests(unittest.TestCase):
    ...
    def sequence_model(self):
        return (RagSequenceForGeneration.
    ...
    def test_rag_token_generate_batch(self):
        ...
        rag_token = RagTokenForGeneration.from_pretrained(
            "facebook/rag-token-nq", retriever=retriever).to(torch_device)
