...
class BlenderbotModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        inputs_dict = prepare_blenderbot_inputs_dict(config, input_ids,
                                                     decoder_input_ids)
...
@require_torch
class BlenderbotModelTest(ModelTesterMixin, GenerationTesterMixin,
                          unittest.TestCase):
    all_model_classes = ((BlenderbotModel, BlenderbotForConditionalGeneration)
    ...
    def test_generate_fp16(self):
        ...
        model = BlenderbotForConditionalGeneration(config).eval().to(
            torch_device)
...
@require_torch
class BlenderbotStandaloneDecoderModelTest(ModelTesterMixin,
                                           GenerationTesterMixin,
                                           unittest.TestCase):
    all_model_classes = ((BlenderbotDecoder,
                          BlenderbotForCausalLM) if is_torch_available() else
                         ())
    all_generative_model_classes = ((BlenderbotForCausalLM, )
                                    if is_torch_available() else ())
    test_pruning = False
    is_encoder_decoder = False
