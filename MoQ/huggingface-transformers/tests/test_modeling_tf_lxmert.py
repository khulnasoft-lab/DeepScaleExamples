...
class TFLxmertModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    all_model_classes = ((TFLxmertModel,
                          TFLxmertForPreTraining) if is_tf_available() else ())
    ...
    def test_attention_outputs(self):
        ...
        encoder_seq_length = (self.model_tester.encoder_seq_length if hasattr(
            self.model_tester, "encoder_seq_length") else
                              self.model_tester.seq_length)
        encoder_key_length = (self.model_tester.key_length if hasattr(
                    ...
                    if isinstance(value, dict):
                        ...
                        if isinstance(value, (list, tuple)):
                            return_dict[key] = (torch.from_numpy(
                                iter_value.numpy()).to(torch_type(key))
    ...
    def test_compile_tf_model(self):
        ...
        for model_class in self.all_model_classes:
            ...
            extended_model = tf.keras.Model(
                inputs=[input_ids, visual_feats, visual_pos],
                outputs=[outputs])
