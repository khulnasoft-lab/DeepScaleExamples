...
class ProphetNetModelTester:
    ...
    def create_and_check_model(
        ...
        self.parent.assertEqual(
            len(decoder_past[0]),
            2)  # cross-attention + uni-directional self-attention
    ...
    def create_and_check_encoder_decoder_shared_weights(
        ...
        for model_class in [
            ...
            if model_class == ProphetNetForConditionalGeneration:
                ...
                model.encoder.load_state_dict(model.decoder.state_dict(),
                                              strict=False)
    ...
    def check_fast_integration(
        ...
        *args,
        ...
        input_ids = torch.tensor([[7, 4, 78, 0, 24, 52, 43]],
                                 device=torch_device,
                                 dtype=torch.long)
        decoder_input_ids = torch.tensor([[12, 62, 25, 11, 47, 15, 14]],
                                         device=torch_device,
                                         dtype=torch.long)
        attention_mask = torch.tensor([[1, 1, 1, 0, 1, 0, 0]],
                                      device=torch_device,
                                      dtype=torch.long)
        decoder_attention_mask = torch.tensor([[1, 1, 1, 0, 0, 1, 0]],
                                              device=torch_device,
                                              dtype=torch.long)
        lm_labels = torch.tensor([[62, 25, 11, 47, 15, 14, 24]],
                                 device=torch_device,
                                 dtype=torch.long)
        torch.manual_seed(0)
        config.ngram = 4
        ...
        self.parent.assertTrue(
            torch.allclose(result.logits[0, :, 1],
                           expected_logit_slice,
                           atol=1e-3))

    def check_model_with_attn_mask(self, config, input_ids, decoder_input_ids,
                                   *args):
        model = ProphetNetModel(config=config)
        model.to(torch_device)
        model.eval()

        outputs_no_mask = model(input_ids=input_ids[:, :5],
    ...
    def check_causal_lm_from_pretrained(
        ...
        *args,
        ...
        model = ProphetNetForConditionalGeneration(config).to(
            torch_device).eval()
        ...
        dec_outputs = decoder(encoder_hidden_states=encoder_hidden_states,
                              input_ids=decoder_input_ids)
...
class ProphetNetStandaloneDecoderModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        lm_labels = None
        if self.use_labels:
            lm_labels = ids_tensor([self.batch_size, self.encoder_seq_length],
                                   self.vocab_size)
...
@require_torch
class ProphetNetModelTest(ModelTesterMixin, GenerationTesterMixin,
                          unittest.TestCase):
    all_model_classes = ((ProphetNetModel, ProphetNetForConditionalGeneration)
                         if is_torch_available() else ())
    all_generative_model_classes = ((ProphetNetForConditionalGeneration, )
                                    if is_torch_available() else ())
    test_pruning = False
    ...
    def test_attention_outputs(self):
        ...
        for model_class in self.all_model_classes:
            ...
            attentions = (outputs.encoder_attentions
                          if config.is_encoder_decoder else outputs.attentions)
...
@require_torch
class ProphetNetStandaloneDecoderModelTest(ModelTesterMixin,
                                           GenerationTesterMixin,
                                           unittest.TestCase):
    all_model_classes = ((ProphetNetDecoder,
                          ProphetNetForCausalLM) if is_torch_available() else
                         ())
    all_generative_model_classes = ((ProphetNetForCausalLM, )
                                    if is_torch_available() else ())
    test_pruning = False
    test_torchscript = False
    test_resize_embeddings = False
...
@require_torch
class ProphetNetStandaloneEncoderModelTest(ModelTesterMixin,
                                           unittest.TestCase):
...
class ProphetNetModelIntegrationTest(unittest.TestCase):
    ...
    def test_pretrained_checkpoint_hidden_states(self):
        ...
        decoder_prev_ids = torch.tensor([[
            102, 2129, 2116, 2372, 2024, 2006, 2169, 1997, 2122, 2048, 2780,
            1029
        ]]).to(torch_device)
        output = model(
        ...
        assert torch.allclose(output_predited_logits[:, :3, :3],
                              expected_slice,
                              atol=1e-4)
        ...
        assert torch.allclose(encoder_outputs[:, :3, :3],
        ...
        predicting_streams = decoder_outputs[1].view(1, model.config.ngram, 12,
                                                     -1)
        ...
        assert torch.allclose(next_first_stream_logits[:, :3, :3],
                              expected_slice,
                              atol=1e-4)
    ...
    def test_question_gen_inference(self):
        ...
        gen_output = model.generate(input_ids,
                                    num_beams=5,
                                    early_stopping=True)
        generated_questions = tokenizer.batch_decode(gen_output,
