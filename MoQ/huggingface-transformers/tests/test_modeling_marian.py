...
@require_torch
class MarianModelTest(ModelTesterMixin, GenerationTesterMixin,
                      unittest.TestCase):
    all_model_classes = (MarianModel,
...
class MarianIntegrationTest(unittest.TestCase):
    ...
    def translate_src_text(self, **tokenizer_kwargs):
        model_inputs = self.tokenizer.prepare_seq2seq_batch(
            src_texts=self.src_text, return_tensors="pt",
            **tokenizer_kwargs).to(torch_device)
        ...
        generated_words = self.tokenizer.batch_decode(generated_ids,
...
class TestMarian_EN_DE_More(MarianIntegrationTest):
    ...
    def test_forward(self):
        ...
        model_inputs: dict = self.tokenizer.prepare_seq2seq_batch(
            src, tgt_texts=tgt, return_tensors="pt").to(torch_device)
    ...
    def test_unk_support(self):
        ...
        ids = (t.prepare_seq2seq_batch(
            ["||"],
            return_tensors="pt").to(torch_device).input_ids[0].tolist())
        expected = [t.unk_token_id, t.unk_token_id, t.eos_token_id]
        self.assertEqual(expected, ids)

    def test_pad_not_split(self):
        input_ids_w_pad = (self.tokenizer.prepare_seq2seq_batch(
...
@require_torch
class MarianStandaloneDecoderModelTest(ModelTesterMixin, GenerationTesterMixin,
                                       unittest.TestCase):
    all_model_classes = ((MarianDecoder,
