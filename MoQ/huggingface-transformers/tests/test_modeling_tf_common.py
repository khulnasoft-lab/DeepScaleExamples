...
class TFModelTesterMixin:
    ...
    def _prepare_for_class(self,
        ...
        if model_class in TF_MODEL_FOR_MULTIPLE_CHOICE_MAPPING.values():
            inputs_dict = {
                ...
                ) if isinstance(v, tf.Tensor) and v.ndim > 0 else v)
        ...
        for model_class in self.all_model_classes:
            ...
            if model.config.is_encoder_decoder:
                ...
                expected_arg_names.extend(
                    ...
                    if "head_mask" and "decoder_head_mask" in arg_names else
                    ["encoder_outputs"])
        ...
        for model_class in self.all_model_classes:
            if self.is_encoder_decoder:
                input_ids = {
                    ...
                    "input_ids":
                    tf.keras.Input(batch_shape=(2, max_input),
                                   name="input_ids",
                                   dtype="int32"),
                }
            elif model_class in TF_MODEL_FOR_MULTIPLE_CHOICE_MAPPING.values():
                input_ids = tf.keras.Input(batch_shape=(4, 2, max_input),
                                           name="input_ids",
                                           dtype="int32")
            else:
                input_ids = tf.keras.Input(batch_shape=(2, max_input),
                                           name="input_ids",
                                           dtype="int32")
            ...
            model(self._prepare_for_class(
                inputs_dict,
                model_class))  # Model must be called before saving.
    ...
    def test_keyword_and_dict_args(self):
        ...
        for model_class in self.all_model_classes:
            ...
            self.assertLess(np.sum(np.abs(output_dict - output_keywords)),
                            1e-6)
    ...
    def test_headmasking(self):
        ...
        for model_class in self.all_model_classes:
            ...
            head_mask = tf.stack(
                [
                    prepare_layer_head_mask(i, config.num_attention_heads,
            ...
            def check_attentions_validity(attentions):
                ...
                for t in attentions:
                    self.assertLess(
                        (tf.math.reduce_sum(
                            tf.cast(tf.math.is_nan(t), tf.float32))).numpy(),
                        (tf.size(t) / 4).numpy(),
                ...
                if (
                    ...
                    self.assertNotEqual(
                        tf.math.reduce_sum(attentions[1][...,
                                                         0, :, :]).numpy(),
                        0.0)
                self.assertAlmostEqual(
        ...
        for model_class in self.all_model_classes:
            for size in [config.vocab_size - 10, config.vocab_size + 10, None]:
                ...
                if old_bias is not None and new_bias is not None:
                    for old_weight, new_weight in zip(old_bias.values(),
                                                      new_bias.values()):
                        self.assertEqual(new_weight.shape[0], assert_size)

                        models_equal = True
                        for p1, p2 in zip(old_weight.value(),
                                          new_weight.value()):
                            if tf.math.reduce_sum(tf.math.abs(p1 - p2)) > 0:
                                models_equal = False
                        self.assertTrue(models_equal)
                ...
                if (old_output_embeddings is not None
                    ...
                    self.assertEqual(new_output_embeddings.shape[0],
                                     assert_size)
                    self.assertEqual(new_output_embeddings.shape[1],
    ...
    def _generate_random_bad_tokens(self, num_bad_tokens, model):
        ...
        while len(bad_tokens) < num_bad_tokens:
            token = tf.squeeze(
                ids_tensor((1, 1), self.model_tester.vocab_size), 0).numpy()[0]
        ...
        for bad_word_ids in bad_words_ids:
            ...
            for generated_ids_slice in generated_ids:
                ...
                for i in range(len(bad_word_ids), len(generated_ids_slice)):
                    # if tokens match
                    if generated_ids_slice[i - len(bad_word_ids
                                                   ):i] == bad_word_ids:
                        return True
        return False
...
class UtilsFunctionsTest(unittest.TestCase):
    ...
    def test_top_k_top_p_filtering(self):
        ...
        output = tf_top_k_top_p_filtering(logits,
                                          top_k=10,
                                          top_p=0.6,
        ...
        tf.debugging.assert_near(non_inf_output,
