...
class LogitsProcessorTest(unittest.TestCase):
    ...
    def test_temperature_dist_warper(self):
        ...
        warped_prob_sharp = F.softmax(temp_dist_warper_sharper(
            input_ids, scores.clone()),
                                      dim=-1)
        warped_prob_smooth = F.softmax(temp_dist_warper_smoother(
            input_ids, scores.clone()),
                                       dim=-1)
    ...
    def test_repetition_penalty_dist_process(self):
        input_ids = torch.tensor([[0, 1], [5, 0]],
                                 device=torch_device,
                                 dtype=torch.long)
        vocab_size = 10
    ...
    def test_top_k_dist_warper(self):
        ...
        ramp_logits = (torch.arange(vocab_size,
                                    device=torch_device,
                                    dtype=torch.float).unsqueeze(0).repeat(
                                        batch_size, 1))
        ramp_logits[1:, :vocab_size //
                    2] = (ramp_logits[1:, :vocab_size // 2] + vocab_size)
        ...
        top_k_warp_safety_check = TopKLogitsWarper(top_k=1,
                                                   filter_value=0.0,
        ...
        ramp_logits = (torch.arange(length,
                                    device=torch_device,
                                    dtype=torch.float).unsqueeze(0).repeat(
                                        batch_size, 1))
    ...
    def test_top_p_dist_warper(self):
        ...
        ramp_logits = torch.arange(vocab_size,
                                   device=torch_device,
                                   dtype=torch.float).unsqueeze(0).repeat(
                                       batch_size, 1) - (vocab_size // 2)

        # make ramp_logits more extreme
        ramp_logits[1] = ramp_logits[1] * 100.0

        # make sure at least 2 tokens are kept
        top_p_warp = TopPLogitsWarper(0.9,
    ...
    def test_no_repeat_ngram_dist_processor(self):
        ...
        input_ids = torch.tensor([[1, 1, 2, 1], [0, 1, 0, 1]],
                                 device=torch_device,
                                 dtype=torch.long)
        scores = self._get_uniform_logits(batch_size, vocab_size)
        ...
        filtered_scores_2_gram = no_repeat_proc_2_gram(input_ids,
                                                       scores.clone())
        filtered_scores_3_gram = no_repeat_proc_3_gram(input_ids,
                                                       scores.clone())
    ...
    def test_encoder_no_repeat_ngram_dist_processor(self):
        ...
        encoder_input_ids = torch.tensor([1, 2, 1, 1],
                                         device=torch_device,
                                         dtype=torch.long)

        input_ids = torch.tensor([[1, 2, 1], [8, 0, 2]],
                                 device=torch_device,
                                 dtype=torch.long)
        ...
        num_beams = 2
        batch_size = 2
        encoder_input_ids = torch.tensor([[1, 2, 1, 1], [0, 0, 2, 1]],
                                         device=torch_device,
                                         dtype=torch.long)
    ...
    def test_hamming_diversity(self):
        ...
        current_tokens = torch.tensor([0, 3, 1, 2],
                                      device=torch_device,
                                      dtype=torch.long)
        ...
        processed_scores = diversity_logits_processor(None, scores,
                                                      current_tokens, 1)

        self.assertTrue(
            torch.allclose(
                processed_scores[0],
                torch.tensor([-0.7500, 0.2500, 0.2500, 0.2500],
                             device=torch_device),
                atol=1e-3,
            ))
        self.assertTrue(
            torch.allclose(
                processed_scores[1],
                torch.tensor([0.2500, -0.7500, 0.2500, 0.2500],
                             device=torch_device),
                atol=1e-3,
            ))
    ...
    def test_forced_bos_token_logits_processor(self):
        ...
        self.assertListEqual(scores[:, bos_token_id].tolist(),
                             4 * [0])  # score for bos_token_id shold be zero
    ...
    def test_forced_eos_token_logits_processor(self):
        ...
        self.assertListEqual(scores[:, eos_token_id].tolist(),
                             4 * [0])  # score for eos_token_id should be zero
