...
class GPT2ModelTester:
    ...
    def create_and_check_gpt2_model_past(self, config, input_ids, input_mask,
        ...
        output_from_no_past = model(
            next_input_ids,
            token_type_ids=next_token_type_ids)["last_hidden_state"]
        output_from_past = model(next_tokens,
                                 token_type_ids=next_token_types,
                                 past_key_values=past)["last_hidden_state"]
    ...
    def create_and_check_gpt2_for_sequence_classification(
            self, config, input_ids, input_mask, head_mask, token_type_ids,
            mc_token_ids, sequence_labels, *args):
...
class GPT2ModelTest(ModelTesterMixin, GenerationTesterMixin,
    ...
    all_generative_model_classes = ((GPT2LMHeadModel, GPT2DoubleHeadsModel)
                                    if is_torch_available() else ())
    all_parallelizable_model_classes = ((GPT2LMHeadModel, )
                                        if is_torch_available() else ())
...
class GPT2ModelLanguageGenerationTest(unittest.TestCase):
    ...
    def test_lm_generate_gpt2(self):
        for checkpointing in [True, False]:
            ...
            model.to(torch_device)
            input_ids = torch.tensor([[464, 3290]],
                                     dtype=torch.long,
                                     device=torch_device)  # The dog
            expected_output_ids = [
                464,
                3290,
                373,
