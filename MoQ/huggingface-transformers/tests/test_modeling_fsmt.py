...
@require_torch
class FSMTModelTest(ModelTesterMixin, GenerationTesterMixin,
                    unittest.TestCase):
    all_model_classes = ((FSMTModel, FSMTForConditionalGeneration)
                         if is_torch_available() else ())
    all_generative_model_classes = ((FSMTForConditionalGeneration, )
                                    if is_torch_available() else ())
