...
WORKING_REPO_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                "fixtures/working_repo")
...
class HfLargefilesTest(HfApiCommonTest):
    ...
    def setup_local_clone(self, REMOTE_URL):
        REMOTE_URL_AUTH = REMOTE_URL.replace(ENDPOINT_STAGING,
    ...
    def test_end_to_end_thresh_6M(self):
        REMOTE_URL = self._api.create_repo(token=self._token,
        ...
        failed_process = subprocess.run(["git", "push"],
                                        capture_output=True,
                                        cwd=WORKING_REPO_DIR)
        ...
        subprocess.run(
            ["transformers-cli", "lfs-enable-largefiles", WORKING_REPO_DIR],
            check=True)
        ...
        dest_filesize = os.stat(os.path.join(WORKING_REPO_DIR,
                                             DEST_FILENAME)).st_size
