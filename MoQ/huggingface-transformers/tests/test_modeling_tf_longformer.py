...
class TFLongformerModelTester:
    ...
    def create_and_check_attention_mask_determinism(
        ...
        tf.debugging.assert_near(output_with_mask[0, 0, :5],
    ...
    def create_and_check_for_multiple_choice(
        ...
        self.parent.assertListEqual(list(output.shape),
...
class TFLongformerModelIntegrationTest(unittest.TestCase):
    ...
    def test_layer_global_attn(self):
        ...
        tf.debugging.assert_near(output_hidden_states[0, 2],
                                 expected_slice_0,
                                 rtol=1e-3)
        tf.debugging.assert_near(output_hidden_states[1, -2],
                                 expected_slice_1,
                                 rtol=1e-3)
    ...
    def test_inference_masked_lm_long(self):
        ...
        # assert close
        tf.debugging.assert_near(tf.reduce_mean(loss),
                                 expected_loss,
                                 rtol=1e-4)
        tf.debugging.assert_near(tf.reduce_sum(prediction_scores),
