...
@require_torch
class BlenderbotSmallModelTest(ModelTesterMixin, GenerationTesterMixin,
                               unittest.TestCase):
    all_model_classes = ((BlenderbotSmallModel,
                          BlenderbotSmallForConditionalGeneration)
                         if is_torch_available() else ())
    all_generative_model_classes = ((BlenderbotSmallForConditionalGeneration, )
                                    if is_torch_available() else ())
    ...
    def test_generate_fp16(self):
        ...
        model = BlenderbotSmallForConditionalGeneration(config).eval().to(
            torch_device)
...
class Blenderbot90MIntegrationTests(unittest.TestCase):
    ...
    def test_90_generation_from_short_input(self):
        model_inputs = self.tokenizer(["sam"],
                                      return_tensors="pt").to(torch_device)
...
@require_torch
class BlenderbotSmallStandaloneDecoderModelTest(ModelTesterMixin,
                                                GenerationTesterMixin,
                                                unittest.TestCase):
    all_model_classes = ((BlenderbotSmallDecoder, BlenderbotSmallForCausalLM)
                         if is_torch_available() else ())
    all_generative_model_classes = ((BlenderbotSmallForCausalLM, )
                                    if is_torch_available() else ())
    test_pruning = False
    is_encoder_decoder = False
