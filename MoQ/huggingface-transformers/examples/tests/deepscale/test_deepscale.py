...
from transformers.integrations import is_deepscale_available
...
def require_deepscale(test_case):
    """
    Decorator marking a test that requires deepscale
    """
    if not is_deepscale_available():
        return unittest.skip("test requires deepscale")(test_case)
    else:
        return test_case
