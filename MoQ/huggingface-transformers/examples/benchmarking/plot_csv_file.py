...
class PlotArguments:
    ...
    plot_along_batch: bool = field(
        ...
        metadata={
            "help":
            "Whether to plot along batch size or sequence length. Defaults to sequence length."
        },
    )
    is_time: bool = field(
        default=False,
    ...
    is_train: bool = field(
        ...
        metadata={
            "help":
            "Whether the csv file has training results or inference results. Defaults to inference results."
        },
    )
        ...
        with open(self.args.csv_file, newline="") as csv_file:
            ...
            for row in reader:
                ...
                if can_convert_to_int(row["result"]):
                    # value is not None
                    self.result_dict[model_name]["result"][(int(
                        row["batch_size"]), int(
                            row["sequence_length"]))] = int(row["result"])
                elif can_convert_to_float(row["result"]):
                    # value is not None
                    self.result_dict[model_name]["result"][(int(
                        row["batch_size"]), int(
                            row["sequence_length"]))] = float(row["result"])

    def plot(self):
        fig, ax = plt.subplots()
        title_str = "Time usage" if self.args.is_time else "Memory usage"
        title_str = (title_str +
                     " for training" if self.args.is_train else title_str +
                     " for inference")
        ...
        for model_name_idx, model_name in enumerate(self.result_dict.keys()):
            ...
            (x_axis_array,
             inner_loop_array) = ((batch_sizes, sequence_lengths)
                                  if self.args.plot_along_batch else
                                  (sequence_lengths, batch_sizes))

            label_model_name = (model_name
                                if self.args.short_model_names is None else
                                self.args.short_model_names[model_name_idx])
            ...
            for inner_loop_value in inner_loop_array:
                ...
                (x_axis_label,
                 inner_loop_label) = (("batch_size", "len")
                                      if self.args.plot_along_batch else
                                      ("in #tokens", "bsz"))

                x_axis_array = np.asarray(x_axis_array,
                                          np.int)[:len(y_axis_array)]
                plt.scatter(
                    x_axis_array,
                    y_axis_array,
                    label=
                    f"{label_model_name} - {inner_loop_label}: {inner_loop_value}",
                )
