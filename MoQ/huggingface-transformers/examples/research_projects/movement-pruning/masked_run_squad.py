...
MODEL_CLASSES = {
    ...
    "masked_bert":
    (MaskedBertConfig, MaskedBertForQuestionAnswering, BertTokenizer),
}
...
def schedule_threshold(
    ...
    if step <= initial_warmup * warmup_steps:
        ...
        mul_coeff = 1 - (step - spars_warmup_steps) / (total_step -
                                                       spars_schedu_steps)
        threshold = final_threshold + (initial_threshold -
                                       final_threshold) * (mul_coeff**3)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if teacher is not None:
                ...
                loss_start = F.kl_div(
                    input=F.log_softmax(start_logits_stu / args.temperature,
                                        dim=-1),
                    target=F.softmax(start_logits_tea / args.temperature,
                                     dim=-1),
                    reduction="batchmean",
                ) * (args.temperature**2)
                loss_end = F.kl_div(
                    input=F.log_softmax(end_logits_stu / args.temperature,
                                        dim=-1),
                    target=F.softmax(end_logits_tea / args.temperature,
                                     dim=-1),
                    reduction="batchmean",
                ) * (args.temperature**2)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if (args.local_rank in [-1, 0] and args.logging_steps > 0
                    ...
                    tb_writer.add_scalar("lr", learning_rate_scalar[0],
                                         global_step)
                    if len(learning_rate_scalar) > 1:
                        for idx, lr in enumerate(learning_rate_scalar[1:]):
                            tb_writer.add_scalar(f"lr/{idx+1}", lr,
                                                 global_step)
                    tb_writer.add_scalar(
                        "loss",
                    ...
                    if teacher is not None:
                        tb_writer.add_scalar("loss/distil", loss_logits.item(),
                                             global_step)
                    if args.regularization is not None:
                        tb_writer.add_scalar("loss/regularization",
                                             regu_.item(), global_step)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    if args.model_type in ["xlnet", "xlm"]:
        start_n_top = (model.config.start_n_top if hasattr(model, "config")
                       else model.module.config.start_n_top)
        end_n_top = (model.config.end_n_top if hasattr(model, "config") else
                     model.module.config.end_n_top)
...
def load_and_cache_examples(args,
    ...
    cached_features_file = os.path.join(
        ...
        "cached_{}_{}_{}_{}".format(
            ...
            (args.tokenizer_name if args.tokenizer_name else list(
                filter(None, args.model_name_or_path.split("/"))).pop()),
            str(args.max_seq_length),
            (list(filter(None, args.predict_file.split("/"))).pop() if evaluate
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if not args.data_dir and ((evaluate and not args.predict_file) or
            ...
            processor = (SquadV2Processor() if args.version_2_with_negative
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "Initialization method for the mask scores. Choices: constant, uniform, kaiming.",
    )
    parser.add_argument(
        "--mask_scale",
        default=0.0,
    ...
    parser.add_argument(
        ...
        type=float,
        help=
        "Regularization intensity (used in conjunction with `regularization`.",
    )

    parser.add_argument("--global_topk",
                        action="store_true",
    ...
    output_eval_file = os.path.join(args.output_dir, predict_file,
                                    "eval_results.txt")
