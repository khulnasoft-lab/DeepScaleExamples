        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if teacher is not None:
                if "token_type_ids" not in inputs:
                    inputs["token_type_ids"] = (None if args.teacher_type
                                                == "xlm" else batch[2])
                with torch.no_grad():
                    (logits_tea, ) = teacher(
                ...
                if "masked" in args.model_type:
                    ...
                    if args.global_topk:
                        if threshold_mem is None:
                            ...
                            n = concat.numel()
                            kth = max(n - (int(n * args.final_threshold) + 1),
                                      1)
                            threshold_mem = concat.kthvalue(kth).values.item()
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "Path to the already fine-tuned teacher model. Only for distillation.",
    )
    parser.add_argument(
        "--alpha_ce",
        default=0.5,
