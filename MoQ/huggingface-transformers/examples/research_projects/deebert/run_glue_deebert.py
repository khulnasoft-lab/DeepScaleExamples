...
MODEL_CLASSES = {
    ...
    "roberta":
    (RobertaConfig, DeeRobertaForSequenceClassification, RobertaTokenizer),
}
        ...
        optimizer_grouped_parameters = [
            {
                "params": [
                    ...
                    if ("highway" in n) and (not any(nd in n
                                                     for nd in no_decay))
                ],
                "weight_decay":
                args.weight_decay,
        ...
        optimizer_grouped_parameters = [
            {
                "params": [
                    p for n, p in model.named_parameters()
                    if ("highway" not in n) and (not any(nd in n
                                                         for nd in no_decay))
                ],
                "weight_decay":
                args.weight_decay,
            },
            {
                "params": [
                    p for n, p in model.named_parameters()
                    if ("highway" not in n) and (any(nd in n
                                                     for nd in no_decay))
                ],
                "weight_decay":
                0.0,
...
def evaluate(args,
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        eval_sampler = (SequentialSampler(eval_dataset) if args.local_rank
                        == -1 else DistributedSampler(eval_dataset))
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if features[0].token_type_ids is None:
        ...
        all_token_type_ids = torch.tensor([[0] * args.max_seq_length
                                           for f in features],
                                          dtype=torch.long)
    else:
