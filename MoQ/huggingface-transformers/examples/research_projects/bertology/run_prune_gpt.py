...
def save_model(model, dirpath):
    ...
    if os.path.exists(dirpath):
        if os.path.exists(os.path.join(
                dirpath, "config.json")) and os.path.isfile(
                    os.path.join(dirpath, "config.json")):
            os.remove(os.path.join(dirpath, "config.json"))
        if os.path.exists(os.path.join(
                dirpath, "pytorch_model.bin")) and os.path.isfile(
    ...
    for step, inputs in enumerate(
        ...
        if compute_entropy:
            for layer, attn in enumerate(all_attentions):
                ...
                attn_entropy[layer] += masked_entropy.sum(-1).sum(0).sum(
                    0).detach()
...
def mask_heads(args, model, eval_dataloader):
    ...
    """
    _, head_importance, loss = compute_heads_importance(args,
                                                        model,
                                                        eval_dataloader,
...
def main():
    ...
    eval_dataloader = DataLoader(train_data,
