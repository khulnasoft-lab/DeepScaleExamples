...
class BertEncoderWithPabee(BertEncoder):
    def adaptive_forward(self,
                         ...
                         attention_mask=None,
                         head_mask=None):
        layer_outputs = self.layer[current_layer](hidden_states,
                                                  attention_mask,
        ...
        if self.training:
            ...
            for i in range(self.config.num_hidden_layers):
                ...
                if regression:
                    ...
                    if (patient_result is not None
                        ) and torch.abs(patient_result -
                                        labels) < self.regression_threshold:
                        patient_counter += 1
                    else:
                        patient_counter = 0
                else:
        ...
        if labels is not None:
            ...
            for ix, logits_item in enumerate(logits):
                if self.num_labels == 1:
                    ...
                    loss = loss_fct(logits_item.view(-1, self.num_labels),
                                    labels.view(-1))
