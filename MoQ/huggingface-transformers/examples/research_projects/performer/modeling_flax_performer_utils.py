...
def generalized_kernel_feature_creator(
    ...
    if projection_matrix is None:
        ...
        data_mod_shape = data.shape[0:len(batch_dims_t
...
def make_fast_softmax_attention(
    ...
    if ortho_features:
        matrix_creator = functools.partial(GaussianOrthogonalRandomMatrix,
                                           nb_features,
                                           qkv_dim,
    ...
    def get_2d_array(self):
        ...
        if self.scaling == 0:
            multiplier = jnp.linalg.norm(random.normal(
                self.key, (self.nb_rows, self.nb_columns)),
                                         axis=1)
        elif self.scaling == 1:
            multiplier = jnp.sqrt(float(self.nb_columns)) * jnp.ones(
                (self.nb_rows))
        else:
            raise ValueError("Scaling must be one of {0, 1}. Was %s" %
                             self._scaling)
