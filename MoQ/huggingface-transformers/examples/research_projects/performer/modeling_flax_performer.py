...
class FlaxPerformerEmbeddings(nn.Module):
    ...
    def __call__(self, input_ids, token_type_ids, position_ids,
                 attention_mask):
        # Embed
        w_emb = FlaxPerformerEmbedding(self.vocab_size,
                                       self.hidden_size,
                                       name="word_embeddings")(jnp.atleast_2d(
                                           input_ids.astype("i4")))
        p_emb = FlaxPerformerEmbedding(self.max_length,
                                       self.hidden_size,
                                       name="position_embeddings")(
                                           jnp.atleast_2d(
                                               position_ids.astype("i4")))
        t_emb = FlaxPerformerEmbedding(self.type_vocab_size,
                                       self.hidden_size,
...
class FlaxPerformerAttention(nn.Module):
    ...
    def __call__(self, hidden_state, attention_mask):
        ...
        layer_norm = FlaxPerformerLayerNorm(name="layer_norm")(self_att +
                                                               hidden_state)
        return layer_norm
...
class FlaxPerformerOutput(nn.Module):
    ...
    def __call__(self, intermediate_output, attention_output):
        hidden_state = nn.Dense(attention_output.shape[-1],
                                name="dense")(intermediate_output)
...
class FlaxPerformerLayer(nn.Module):
    ...
    @nn.compact
    def __call__(self, hidden_state, attention_mask):
        attention = FlaxPerformerAttention(self.num_heads,
                                           self.head_size,
                                           name="attention")(hidden_state,
                                                             attention_mask)
        intermediate = FlaxPerformerIntermediate(
            self.intermediate_size,
            name="intermediate",
            hidden_act=self.hidden_act)(attention)
