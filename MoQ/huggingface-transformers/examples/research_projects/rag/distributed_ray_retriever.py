...
class RagRayDistributedRetriever(RagRetriever):
    ...
    def __init__(
        ...
        if index is not None and index.is_initialized(
        ...
        if len(self.retrieval_workers) > 0:
            ray.get([
                worker.create_rag_retriever.remote(config,
                                                   question_encoder_tokenizer,
                                                   generator_tokenizer, index)
    ...
    def get_tokenizers(cls,
                       ...
                       **kwargs):
        return super(RagRayDistributedRetriever,
                     cls).get_tokenizers(retriever_name_or_path,
                                         indexed_dataset, **kwargs)
