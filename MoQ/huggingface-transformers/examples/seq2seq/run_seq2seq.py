...
class DataTrainingArguments:
    ...
    task: str = field(
        ...
        metadata={
            "help":
            "The name of the task, should be summarization (or summarization_{dataset} for evaluating "
    ...
    validation_file: Optional[str] = field(
        ...
        metadata={
            "help":
            "An optional input evaluation data file to evaluate the metrics (rouge/sacreblue) on "
    ...
    test_file: Optional[str] = field(
        ...
        metadata={
            "help":
            "An optional input test data file to evaluate the metrics (rouge/sacreblue) on "
            "(a jsonlines or csv file)."
        },
    )
    ...
    def __post_init__(self):
        ...
        if not self.task.startswith(
                "summarization") and not self.task.startswith("translation"):
            raise ValueError(
...
def main():
    ...
    if data_args.task.startswith("translation") or isinstance(
            tokenizer, (MBartTokenizer, MBartTokenizerFast)):
    ...
    if data_args.task.startswith("summarization"):
        ...
        if data_args.summary_column is None:
            summary_column = (dataset_columns[1] if dataset_columns is not None
                              else column_names[1])
        else:
