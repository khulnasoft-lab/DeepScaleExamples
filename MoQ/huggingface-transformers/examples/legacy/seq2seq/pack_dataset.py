...
def pack_examples(tok, src_examples, tgt_examples, max_tokens=1024):
    ...
    for src, tgt in tqdm(sorted_examples[1:]):
        ...
        if is_too_big(cand_src) or is_too_big(
                cand_tgt):  # cant fit, finalize example
...
def pack_data_dir(tok, data_dir: Path, max_tokens, save_path):
    ...
    for split in ["train"]:
        ...
        packed_src, packed_tgt = pack_examples(tok, src_docs, tgt_docs,
                                               max_tokens)
        print(
...
def packer_cli():
    ...
    return pack_data_dir(tokenizer, Path(args.data_dir), args.max_seq_len,
                         args.save_path)
