...
class UserCommands(BaseTransformersCLICommand):
    ...
    def register_subcommand(parser: ArgumentParser):
        ...
        rm_parser.add_argument("--organization",
                               type=str,
                               help="Optional: organization namespace.")
        rm_parser.set_defaults(func=lambda args: DeleteObjCommand(args))
        upload_parser = s3_subparsers.add_parser("upload",
        ...
        upload_parser.add_argument("--organization",
                                   type=str,
                                   help="Optional: organization namespace.")
        upload_parser.add_argument(
            "--filename",
...
class ListObjsCommand(BaseUserCommand):
    def run(self):
        ...
        print(
            tabulate(rows,
                     headers=["Filename", "LastModified", "ETag", "Size"]))
    ...
    def run(self):
        ...
        if not self.args.yes:
            ...
            if not (choice == "" or choice == "y" or choice == "yes"):
                ...
                exit()
        try:
            url = self._api.create_repo(token,
                                        name=self.args.name,
                                        organization=self.args.organization)
        except HTTPError as e:
            print(e)
...
class UploadCommand(BaseUserCommand):
    def walk_dir(self, rel_path):
        ...
        files = [(os.path.join(os.getcwd(), f.path), f.path) for f in entries
                 if f.is_file()]  # (filepath, filename)
        for f in entries:
            if f.is_dir():
    ...
    def run(self):
        ...
        if os.path.isdir(local_path):
            ...
            filename = (self.args.filename if self.args.filename is not None
        ...
        for filepath, filename in files:
            print(
                ...
                .format(ANSI.bold(filepath), ANSI.bold(filename),
                        ANSI.bold(namespace)))
