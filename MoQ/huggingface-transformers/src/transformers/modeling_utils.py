...
def get_parameter_device(parameter: Union[nn.Module, GenerationMixin,
                                          "ModuleUtilsMixin"]):
    try:
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin):
    ...
    def _get_resized_embeddings(
        ...
        new_embeddings = nn.Embedding(new_num_tokens,
                                      old_embedding_dim).to(self.device)
    ...
    def _get_resized_lm_head(
        ...
        new_lm_head = nn.Linear(*new_lm_head_shape,
                                bias=has_new_lm_head_bias).to(self.device)
    ...
    def save_pretrained(self, save_directory: Union[str, os.PathLike]):
        ...
        if getattr(self.config, "xla_device",
                   False) and is_torch_tpu_available():
...
class SQuADHead(nn.Module):
    ...
    @replace_return_docstrings(output_type=SquadHeadOutput,
    ...
    def forward(
        ...
        if start_positions is not None and end_positions is not None:
            ...
            return SquadHeadOutput(
                loss=total_loss) if return_dict else (total_loss, )
