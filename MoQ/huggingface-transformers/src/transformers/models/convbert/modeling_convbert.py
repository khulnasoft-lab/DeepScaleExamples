...
def load_tf_weights_in_convbert(model, config, tf_checkpoint_path):
    ...
    for j in range(config.num_hidden_layers):
        ...
        param_mapping[
            f"encoder.layer.{j}.attention.self.key_conv_attn_layer.depthwise.weight"] = f"electra/encoder/layer_{j}/attention/self/conv_attn_key/depthwise_kernel"
        param_mapping[
            f"encoder.layer.{j}.attention.self.key_conv_attn_layer.pointwise.weight"] = f"electra/encoder/layer_{j}/attention/self/conv_attn_key/pointwise_kernel"
        param_mapping[
...
class SeparableConv1D(nn.Module):
    ...
    def __init__(self, config, input_filters, output_filters, kernel_size,
        ...
        self.pointwise = nn.Conv1d(input_filters,
                                   output_filters,
                                   kernel_size=1,
                                   bias=False)
        self.bias = nn.Parameter(torch.zeros(output_filters, 1))

        self.depthwise.weight.data.normal_(mean=0.0,
                                           std=config.initializer_range)
        self.pointwise.weight.data.normal_(mean=0.0,
                                           std=config.initializer_range)
...
class ConvBertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.key_conv_attn_layer = SeparableConv1D(config, config.hidden_size,
                                                   self.all_head_size,
    ...
    def forward(
        ...
        conv_attn_layer = torch.multiply(mixed_key_conv_attn_layer,
                                         mixed_query_layer)

        conv_kernel_layer = self.conv_kernel_layer(conv_attn_layer)
        conv_kernel_layer = torch.reshape(conv_kernel_layer,
                                          [-1, self.conv_kernel_size, 1])
        conv_kernel_layer = torch.softmax(conv_kernel_layer, dim=1)

        conv_out_layer = self.conv_out_layer(hidden_states)
        conv_out_layer = torch.reshape(conv_out_layer,
                                       [batch_size, -1, self.all_head_size])
        conv_out_layer = conv_out_layer.transpose(1,
...
class GroupedLinearLayer(nn.Module):
    def __init__(self, input_size, output_size, num_groups):
        ...
        self.weight = nn.Parameter(
            torch.Tensor(self.num_groups, self.group_in_dim,
                         self.group_out_dim))
        self.bias = nn.Parameter(torch.Tensor(output_size))

    def forward(self, hidden_states):
        batch_size = list(hidden_states.size())[0]
        x = torch.reshape(hidden_states,
