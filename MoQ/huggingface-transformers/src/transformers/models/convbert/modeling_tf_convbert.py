...
class TFConvBertSelfAttention(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        new_num_attention_heads = int(config.num_attention_heads /
                                      config.head_ratio)
    ...
    def call(
        ...
        conv_attn_layer = tf.multiply(mixed_key_conv_attn_layer,
                                      mixed_query_layer)
        ...
        conv_out_layer = tf.reshape(conv_out_layer,
        ...
        unfold_conv_out_layer = tf.stack(
            [
                tf.slice(
                    ...
                    [
                        batch_size,
                        shape_list(mixed_query_layer)[1], self.all_head_size
                    ],
        ...
        dk = tf.cast(shape_list(key_layer)[-1],
                     attention_scores.dtype)  # scale attention_scores
...
class GroupedLinearLayer(tf.keras.layers.Layer):
    def __init__(self, input_size, output_size, num_groups, kernel_initializer,
                 **kwargs):
        super().__init__(**kwargs)
...
class TFConvBertLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        layer_output = self.bert_output(intermediate_output,
                                        attention_output,
                                        training=training)
...
class TFConvBertForMaskedLM(TFConvBertPreTrainedModel,
                            TFMaskedLanguageModelingLoss):
    def __init__(self, config, *inputs, **kwargs):
...
class TFConvBertForSequenceClassification(TFConvBertPreTrainedModel,
                                          TFSequenceClassificationLoss):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.num_labels = config.num_labels
        self.convbert = TFConvBertMainLayer(config, name="convbert")
        self.classifier = TFConvBertClassificationHead(config,
                                                       name="classifier")
...
class TFConvBertForMultipleChoice(TFConvBertPreTrainedModel,
                                  TFMultipleChoiceLoss):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
...
class TFConvBertForTokenClassification(TFConvBertPreTrainedModel,
                                       TFTokenClassificationLoss):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
...
class TFConvBertForQuestionAnswering(TFConvBertPreTrainedModel,
                                     TFQuestionAnsweringLoss):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
