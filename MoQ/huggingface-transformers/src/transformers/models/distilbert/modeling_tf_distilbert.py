...
class TFMultiHeadSelfAttention(tf.keras.layers.Layer):
    ...
    def call(self,
        ...
        q = tf.multiply(q,
                        tf.math.rsqrt(tf.cast(dim_per_head, dtype=tf.float32)))
        k = tf.cast(k, dtype=q.dtype)
        scores = tf.matmul(
            q, k, transpose_b=True)  # (bs, n_heads, q_length, k_length)
...
class TFFFN(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        assert config.activation in [
            "relu",
            "gelu",
        ], "activation ({}) must be in ['relu', 'gelu']".format(
            config.activation)
...
class TFTransformerBlock(tf.keras.layers.Layer):
    ...
    def call(self,
        ...
        ffn_output = self.ffn(sa_output,
                              training=training)  # (bs, seq_length, dim)
        ffn_output = self.output_layer_norm(ffn_output +
                                            sa_output)  # (bs, seq_length, dim)
    ...
    def call(
        ...
        **kwargs,
        ...
        embedding_output = self.embeddings(
            inputs["input_ids"],
            inputs_embeds=inputs["inputs_embeds"])  # (bs, seq_length, dim)
...
class TFDistilBertForMaskedLM(TFDistilBertPreTrainedModel,
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.vocab_projector = TFDistilBertLMHead(config,
                                                  self.distilbert.embeddings,
                                                  name="vocab_projector")
    ...
    def call(
        ...
        **kwargs,
        ...
        pooled_output = self.dropout(pooled_output,
                                     training=inputs["training"])  # (bs, dim)
...
class TFDistilBertForMultipleChoice(TFDistilBertPreTrainedModel,
                                    TFMultipleChoiceLoss):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
    ...
    def call(
        ...
        **kwargs,
        ...
        hidden_states = self.dropout(
            hidden_states,
            training=inputs["training"])  # (bs, max_query_len, dim)
