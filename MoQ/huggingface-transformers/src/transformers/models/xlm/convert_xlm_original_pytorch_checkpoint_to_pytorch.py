...
def convert_xlm_checkpoint_to_pytorch(xlm_checkpoint_path,
    ...
    config = chkpt["params"]
    config = dict((n, v) for n, v in config.items()
                  if not isinstance(v, (torch.FloatTensor, numpy.ndarray)))

    vocab = chkpt["dico_word2id"]
    vocab = dict(
        (s + "</w>" if s.find("@@") == -1 and i > 13 else s.replace("@@", ""),
         i) for s, i in vocab.items())
...
if __name__ == "__main__":
    ...
    convert_xlm_checkpoint_to_pytorch(args.xlm_checkpoint_path,
