...
class LegacyIndex(Index):
    ...
    def _load_passages(self):
        ...
        passages_path = self._resolve_path(self.index_path,
                                           self.PASSAGE_FILENAME)
    ...
    def get_top_docs(self,
        ...
        ids = [[int(self.index_id_to_db_id[doc_id]) for doc_id in doc_ids]
               for doc_ids in docs_ids]
...
class HFIndexBase(Index):
    ...
    def _check_dataset_format(self, with_index: bool):
        ...
        if len({"title", "text", "embeddings"} -
               set(self.dataset.column_names)) > 0:
            raise ValueError(
    ...
    def get_top_docs(self,
                     question_hidden_states: np.ndarray,
                     n_docs=5) -> Tuple[np.ndarray, np.ndarray]:
        _, ids = self.dataset.search_batch("embeddings",
        ...
        return np.array(ids), np.array(
            vectors)  # shapes (batch_size, n_docs) and (batch_size, n_docs, d)
...
class CanonicalHFIndex(HFIndexBase):
    ...
    def init_index(self):
        if self.index_path is not None:
            ...
            logger.info("Loading index from {}".format(self.dataset_name +
                                                       " with index name " +
                                                       self.index_name))
...
class CustomHFIndex(HFIndexBase):
    ...
    def __init__(self, vector_size: int, dataset, index_path=None):
        super().__init__(vector_size,
                         dataset,
                         index_initialized=index_path is None)
        self.index_path = index_path
...
class RagRetriever:
    ...
    def save_pretrained(self, save_directory):
        if isinstance(self.index, CustomHFIndex):
            if self.config.index_path is None:
                index_path = os.path.join(save_directory,
    ...
    def postprocess_docs(self,
        ...
        def cat_input_and_doc(doc_title, doc_text, input_string, prefix):
            ...
            if prefix is None:
                prefix = ""
            out = (prefix + doc_title + self.config.title_sep + doc_text +
                   self.config.doc_sep + input_string).replace("  ", " ")
            return out
        ...
        rag_input_strings = [
            cat_input_and_doc(
                ...
                input_strings[i],
                prefix,
            ) for i in range(len(docs)) for j in range(n_docs)
        ]
    ...
    def _main_retrieve(self, question_hidden_states: np.ndarray,
        ...
        for question_hidden_states in question_hidden_states_batched:
            ...
            ids, vectors = self.index.get_top_docs(question_hidden_states,
                                                   n_docs)
            logger.debug("index search time: {} sec, batch size {}".format(
