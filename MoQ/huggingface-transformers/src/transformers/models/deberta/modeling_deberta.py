...
class DisentangledSelfAttention(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        self.in_proj = torch.nn.Linear(config.hidden_size,
        ...
        self.pos_att_type = (config.pos_att_type
                             if config.pos_att_type is not None else [])

        self.relative_attention = getattr(config, "relative_attention", False)
        self.talking_head = getattr(config, "talking_head", False)

        if self.talking_head:
            self.head_logits_proj = torch.nn.Linear(config.num_attention_heads,
                                                    config.num_attention_heads,
                                                    bias=False)
        ...
        if self.relative_attention:
            ...
            if "c2p" in self.pos_att_type or "p2p" in self.pos_att_type:
                self.pos_proj = torch.nn.Linear(config.hidden_size,
                                                self.all_head_size,
                                                bias=False)
            if "p2c" in self.pos_att_type or "p2p" in self.pos_att_type:
                self.pos_q_proj = torch.nn.Linear(config.hidden_size,
                                                  self.all_head_size)
    ...
    def forward(
        ...
        if query_states is None:
            ...
            k, v = [
                linear(qkvw[i], qkvb[i], hidden_states) for i in range(1, 3)
            ]
    ...
    def disentangled_att_bias(self, query_layer, key_layer, relative_pos,
        ...
        if "c2p" in self.pos_att_type:
            c2p_att = torch.matmul(query_layer,
        ...
        # position->content
        if "p2c" in self.pos_att_type or "p2p" in self.pos_att_type:
            pos_query_layer /= math.sqrt(
                pos_query_layer.size(-1) * scale_factor)
        ...
        if "p2c" in self.pos_att_type:
            p2c_att = torch.matmul(key_layer,
