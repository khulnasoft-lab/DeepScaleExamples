...
class TFAlbertAttention(tf.keras.layers.Layer):
    ...
    def call(
        ...
        attention_probs = self.attention_dropout(inputs=attention_probs,
                                                 training=training)
        ...
        context_layer = tf.reshape(tensor=context_layer,
...
class TFAlbertLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        hidden_states = self.full_layer_layer_norm(inputs=ffn_output +
                                                   attention_outputs[0])
...
class TFAlbertLayerGroup(tf.keras.layers.Layer):
    ...
    def call(
        ...
        return tuple(
            v for v in [hidden_states, layer_hidden_states, layer_attentions]
            if v is not None)
...
class TFAlbertTransformer(tf.keras.layers.Layer):
    def __init__(self, config: AlbertConfig, **kwargs):
        ...
        self.layers_per_group = int(config.num_hidden_layers /
        ...
        self.albert_layer_groups = [
            TFAlbertLayerGroup(config,
                               name="albert_layer_groups_._{}".format(i))
...
class TFAlbertMLMHead(tf.keras.layers.Layer):
    ...
    def call(self, hidden_states: tf.Tensor) -> tf.Tensor:
        ...
        hidden_states = tf.nn.bias_add(value=hidden_states,
...
class TFAlbertForPreTraining(TFAlbertPreTrainedModel, TFAlbertPreTrainingLoss):
    ...
    def __init__(self, config: AlbertConfig, *inputs, **kwargs):
        ...
        self.predictions = TFAlbertMLMHead(
            config,
            input_embeddings=self.albert.embeddings,
            name="predictions")
    ...
    @add_start_docstrings_to_model_forward(
        ALBERT_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @replace_return_docstrings(output_type=TFAlbertForPreTrainingOutput,
                               config_class=_CONFIG_FOR_DOC)
    def call(
        self,
        input_ids: Optional[TFModelInputType] = None,
        ...
        **kwargs,
        ...
        sop_scores = self.sop_classifier(pooled_output=pooled_output,
                                         training=inputs["training"])
        total_loss = None

        if inputs["labels"] is not None and inputs[
                "sentence_order_label"] is not None:
...
class TFAlbertSOPHead(tf.keras.layers.Layer):
    ...
    def call(self, pooled_output: tf.Tensor, training: bool) -> tf.Tensor:
        dropout_pooled_output = self.dropout(inputs=pooled_output,
                                             training=training)
...
class TFAlbertForMultipleChoice(TFAlbertPreTrainedModel, TFMultipleChoiceLoss):
    ...
    def call(
        ...
        **kwargs,
        ...
        flat_position_ids = (tf.reshape(tensor=position_ids,
                                        shape=(-1, seq_length))
