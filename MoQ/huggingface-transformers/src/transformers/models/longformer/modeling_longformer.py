...
def _get_question_end_index(input_ids, sep_token_id):
    ...
    assert sep_token_indices.shape[
...
def _compute_global_attention_mask(input_ids,
                                   sep_token_id,
    ...
    if before_sep_token is True:
        ...
        attention_mask = (attention_mask.expand_as(input_ids) >
                          (question_end_index + 1)).to(torch.uint8) * (
                              attention_mask.expand_as(input_ids) <
                              input_ids.shape[-1]).to(torch.uint8)
...
class LongformerSelfAttention(nn.Module):
    ...
    def _mask_invalid_locations(input_tensor,
        ...
        beginning_input.masked_fill_(
            beginning_mask == 1,
            -float("inf"))  # `== 1` converts to bool or uint8
    ...
    def _sliding_chunks_query_key_matmul(self, query: torch.Tensor,
        ...
        diagonal_attention_scores[:, 0, 1:window_overlap, 1:window_overlap] = (
            diagonal_chunked_attention_scores[:, 0, :window_overlap - 1,
                                              1 - window_overlap:])
...
class LongformerLayer(nn.Module):
    ...
    def forward(
        ...
        layer_output = apply_chunking_to_forward(self.ff_chunk,
...
class LongformerEncoder(nn.Module):
    ...
    def forward(
        ...
        all_attentions = (
        ) if output_attentions else None  # All local attentions.
        all_global_attentions = () if (output_attentions
                                       and is_global_attn) else None
        ...
        for idx, layer_module in enumerate(self.layer):
            ...
            if output_attentions:
                ...
                all_attentions = all_attentions + (layer_outputs[1].transpose(
                    1, 2), )
...
class LongformerModel(LongformerPreTrainedModel):
    ...
    def _pad_to_window_size(
        ...
        # padding
        attention_window = (self.config.attention_window if isinstance(
            self.config.attention_window, int) else max(
        ...
        if padding_len > 0:
            ...
            if position_ids is not None:
                ...
                position_ids = F.pad(position_ids, (0, padding_len),
                                     value=pad_token_id)
            ...
            token_type_ids = F.pad(token_type_ids, (0, padding_len),
    ...
    def forward(
        ...
        extended_attention_mask: torch.Tensor = self.get_extended_attention_mask(
            attention_mask, input_shape, device)[:, 0, 0, :]
...
class LongformerForMultipleChoice(LongformerPreTrainedModel):
    ...
    def forward(
        ...
        flat_global_attention_mask = (global_attention_mask.view(
            -1, global_attention_mask.size(-1)) if global_attention_mask
