...
def _stable_argsort(vector, dim):
    ...
    scale_offset = torch.arange(vector.shape[dim],
                                device=vector.device).view(1, 1, -1)
    scale_offset = scale_offset.expand(vector.shape)
    scaled_vector = vector.shape[dim] * vector + (scale_offset %
                                                  vector.shape[dim])
...
def _get_least_common_mult_chunk_len(config):
    ...
    if len(attn_types_set) == 1 and attn_types[0] == "lsh":
        ...
        return np.lcm(config.lsh_attn_chunk_length,
...
def _get_min_chunk_len(config):
    ...
    if len(attn_types_set) == 1 and attn_types[0] == "lsh":
        ...
        return min(config.lsh_attn_chunk_length,
                   config.local_attn_chunk_length)
    else:
        raise NotImplementedError(
            "Only attn layer types 'lsh' and 'local' exist, but `config.attn_layers`: {}. Select attn layer types from ['lsh', 'local'] only."
...
class AxialPositionEmbeddings(nn.Module):
    ...
    def forward(self, position_ids):
        ...
        broadcasted_weights = [
            weight.expand((batch_size, ) + self.axial_pos_shape +
        ...
        if self.training is True:
            ...
            position_encodings = torch.cat(
                [
                    torch.index_select(position_encodings[i], 0,
                                       position_ids[i]).unsqueeze(0)
...
class PositionEmbeddings(nn.Module):
    ...
    def forward(self, position_ids):
        ...
        position_embeddings = nn.functional.dropout(position_embeddings,
                                                    p=self.dropout,
...
class ReformerEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.position_embeddings = (AxialPositionEmbeddings(config)
                                    if config.axial_pos_embds else
                                    PositionEmbeddings(config))
    ...
    def forward(
        ...
        # dropout
        embeddings = nn.functional.dropout(inputs_embeds,
                                           p=self.dropout,
                                           training=self.training)
...
class LSHSelfAttention(nn.Module, EfficientAttentionMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        do_standard_self_attention = (
            sequence_length <= self.chunk_length) or (
        ...
        if not do_standard_self_attention:
            ...
            sorted_bucket_idx_per_hash = torch.arange(
                sequence_length, device=query_key_vectors.device).repeat(
    ...
    def _query_per_attn_head(self, hidden_states):
        per_head_query_key = self.query_key.weight.reshape(
            ...
            self.hidden_size).transpose(-2, -1)
    ...
    def _get_sorted_bucket_idx_and_undo_sorted_bucket_idx(
        ...
        with torch.no_grad():
            ...
            indices = (torch.arange(sorted_bucket_idx.shape[-1],
                                    device=buckets.device).view(
                                        1, 1,
                                        -1).expand(sorted_bucket_idx.shape))
    ...
    def _attend(
        ...
        query_key_dots = torch.matmul(query_vectors,
                                      key_vectors.transpose(-1, -2))
        ...
        if not do_standard_self_attention:
            ...
            query_bucket_idx = (
                key_value_bucket_idx.new_ones(key_value_bucket_idx.shape[:-1] +
                                              (1, )) *
                key_value_bucket_idx.max())
        elif do_cached_attention and query_key_dots.ndim <= 4:
            query_bucket_idx = (query_key_dots.shape[-1] -
                                1) * torch.ones_like(query_key_dots)[:, :, :,
                                                                     -1]
            key_value_bucket_idx = torch.arange(
                query_key_dots.shape[-1],
                dtype=torch.long,
                device=query_key_dots.device)[None, None, :].expand(
        ...
        self_mask = torch.ne(query_bucket_idx.unsqueeze(-1),
                             key_value_bucket_idx.unsqueeze(-2)).to(
                                 query_bucket_idx.device)

        # apply self_mask
        query_key_dots = torch.where(self_mask, query_key_dots,
                                     self_mask_value)
    ...
    def _compute_attn_mask(
        ...
        if self.is_decoder is True:
            causal_mask = torch.ge(query_indices.unsqueeze(-1),
                                   key_indices.unsqueeze(-2)).to(
    ...
    def _expand_to_indices_in_relevant_chunk(self, indices, sequence_length):
        ...
        chunk_sequence_indices = expanded_start_indices + torch.arange(
            total_chunk_size,
            device=indices.device, dtype=torch.long).unsqueeze(0).expand(
                indices.shape[0], total_chunk_size)
    ...
    def _gather_by_expansion(self, vectors, idxs, num_hashes):
        ...
        """
        expanded_idxs = idxs.unsqueeze(-1).expand(-1, -1, -1,
                                                  self.attention_head_size)
...
class ReverseSort(Function):
    ...
    def backward(ctx, grad_out_vectors, grad_logits):
        ...
        grad_out_vectors = torch.gather(grad_out_vectors, 2,
    ...
    def forward(
        ...
        **kwargs,
        ...
        indices = torch.arange(sequence_length,
                               device=query_vectors.device).repeat(
                                   batch_size, self.num_attention_heads, 1)
    ...
    def __init__(self, config, layer_id=0):
        ...
        if len(set(self.attn_layers)) == 1 and self.attn_layers[0] == "lsh":
            self.self_attention = LSHSelfAttention(config)
                ...
                self.attn_layers)) == 1 and self.attn_layers[0] == "local":
...
class _ReversibleFunction(Function):
    ...
    def forward(
        ...
        for layer_id, (layer,
                       layer_head_mask) in enumerate(zip(layers, head_mask)):
...
class ReformerOnlyLMHead(nn.Module):
    def __init__(self, config):
        ...
        self.decoder = nn.Linear(2 * config.hidden_size,
                                 config.vocab_size,
                                 bias=False)
...
class ReformerModel(ReformerPreTrainedModel):
    ...
    def forward(
        ...
        if must_pad_to_match_chunk_length:
            ...
            if self.training is True:
                raise ValueError(
                    "If training, sequence Length {} has to be a multiple of least common multiple chunk_length {}. Please consider padding the input to a length of {}."
                    .format(
                        input_shape[-1],
    ...
    def _pad_to_mult_of_chunk_length(
        ...
        if attention_mask is not None:
            ...
            attention_mask = torch.cat([attention_mask, pad_attention_mask],
                                       dim=-1)
        else:
        ...
        if inputs_embeds is not None:
            padded_inputs_embeds = self.embeddings(padded_input_ids,
                                                   position_ids)
            inputs_embeds = torch.cat([inputs_embeds, padded_inputs_embeds],
                                      dim=-2)
...
class ReformerForMaskedLM(ReformerPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            masked_lm_loss = loss_fct(logits.view(-1, self.config.vocab_size),
                                      labels.view(-1))
