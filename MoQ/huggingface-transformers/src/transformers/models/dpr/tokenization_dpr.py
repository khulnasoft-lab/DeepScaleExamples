...
DPRSpanPrediction = collections.namedtuple(
    "DPRSpanPrediction",
    [
        "span_score", "relevance_score", "doc_id", "start_index", "end_index",
        "text"
    ],
)
...
class CustomDPRReaderTokenizerMixin:
    def __call__(self,
        ...
        encoded_texts = super().__call__(texts,
                                         add_special_tokens=False,
                                         padding=False,
                                         truncation=False)["input_ids"]
        encoded_inputs = {
            "input_ids":
            [((encoded_question_and_title +
               encoded_text)[:max_length] if max_length is not None
              and truncation else encoded_question_and_title + encoded_text)
    ...
    def decode_best_spans(
        ...
        sorted_docs = sorted(range(n_passages),
                             reverse=True,
            ...
            for start_index, end_index in best_spans:
                ...
                nbest_spans_predictions.append(
                    DPRSpanPrediction(
                        ...
                        end_index=end_index,
                        text=self.decode(sequence_ids[start_index:end_index +
                                                      1]),
                    ))
    ...
    def _get_best_spans(
        ...
        for (start_index, end_index), score in scores:
            ...
            if any([
                    start_index <= prev_start_index <= prev_end_index <=
                    end_index or prev_start_index <= start_index <= end_index
                    <= prev_end_index
                    for (prev_start_index,
                         prev_end_index) in chosen_span_intervals
            ]):
                continue
