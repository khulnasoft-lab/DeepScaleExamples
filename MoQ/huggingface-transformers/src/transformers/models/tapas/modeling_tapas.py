...
class TapasEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        for i, type_vocab_sizes in enumerate(config.type_vocab_sizes):
            ...
            setattr(self, name,
                    nn.Embedding(type_vocab_sizes, config.hidden_size))
    ...
    def forward(self,
        ...
        if position_ids is None:
            ...
            if self.config.reset_position_index_per_cell:
                ...
                # shape (1, seq_len)
                position = torch.arange(seq_length,
                                        dtype=torch.long,
            ...
            if getattr(self.config, "gradient_checkpointing", False):
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        return module(*inputs, past_key_values,
                                      output_attentions)
...
class TapasForQuestionAnswering(TapasPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(
        TAPAS_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @replace_return_docstrings(output_type=TableQuestionAnsweringOutput,
                               config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids=None,
        ...
        row_index = IndexMap(
            indices=torch.min(
                row_ids,
                torch.as_tensor(self.config.max_num_rows - 1,
        ...
        col_index = IndexMap(
            indices=torch.min(
                column_ids,
                torch.as_tensor(self.config.max_num_columns - 1,
                                device=column_ids.device),
            ),
        ...
        if labels is not None:
            calculate_loss = True
            is_supervised = (not self.config.num_aggregation_labels > 0
            ...
            if not self.config.select_one_column:
                ...
                selection_loss_per_example = torch.sum(
                    selection_loss_per_token * input_mask_float,
                    dim=1) / (torch.sum(input_mask_float, dim=1) +
                              EPSILON_ZERO_DIVISION)
            else:
                selection_loss_per_example, logits = _single_column_cell_selection_loss(
                    logits, column_logits, labels, cell_index, col_index,
                    cell_mask)
                dist_per_token = torch.distributions.Bernoulli(logits=logits)
            ...
            if self.config.num_aggregation_labels > 0:
                if is_supervised:
                    ...
                    aggregation_labels = torch.zeros(labels.shape[0],
                                                     dtype=torch.long,
...
class IndexMap(object):
    ...
    def batch_shape(self):
        return self.indices.size()[:self.
                                   batch_dims]  # returns a torch.Size object
...
class ProductIndexMap(IndexMap):
    ...
    def project_inner(self, index):
        ...
        return IndexMap(
            indices=torch.fmod(index.indices,
                               self.inner_index.num_segments).type(
                                   torch.float).floor().type(torch.long),
...
def flatten(index, name="segmented_flatten"):
    ...
    offset = (torch.arange(
        start=0, end=batch_size, device=index.num_segments.device) *
              index.num_segments)
    offset = offset.view(index.batch_shape())
    for _ in range(index.batch_dims,
                   len(index.indices.size())):  # typically range(1,2)
...
def compute_column_logits(
    ...
    column_logits, out_index = reduce_sum(cell_logits * cell_mask,
                                          column_index)
...
def _single_column_cell_selection_loss(token_logits, column_logits, labels,
    ...
    column_label = torch.argmax(labels_per_column,
    ...
    no_cell_selected = torch.eq(
        torch.max(labels_per_column, dim=-1)[0],
        0)  # no_cell_selected is of shape (batch_size,) and equals True
    ...
    selected_column_mask = torch.as_tensor(
        torch.eq(column_id_for_cells,
                 torch.unsqueeze(selected_column_id, dim=-1)),
        dtype=torch.float32,
...
def compute_token_logits(sequence_output, temperature, output_weights,
                         output_bias):
    """
...
def _calculate_aggregate_mask(answer, pooled_output, cell_selection_preference,
                              labels, aggregation_classifier):
    """
    ...
    aggregate_mask_init = (torch.logical_not(torch.isnan(answer)).type(
        torch.FloatTensor).to(answer.device))
    ...
    aggregation_ops_total_mass = torch.sum(dist_aggregation.probs[:, 1:],
                                           dim=1)
    ...
    aggregate_mask = torch.where(
        torch.logical_and(is_pred_cell_selection,
                          is_cell_supervision_available).view(
...
def _calculate_aggregation_loss_known(
    ...
    one_hot_labels = torch.nn.functional.one_hot(
        target_aggregation,
        num_classes=num_aggregation_labels).type(torch.float32)
...
def _calculate_expected_result(
    ...
    numeric_values_masked = torch.where(
        torch.isnan(numeric_values), torch.zeros_like(numeric_values),
        numeric_values)  # Mask non-numeric table values to zero.
    sum_result = torch.sum(scaled_probability_per_cell * numeric_values_masked,
                           dim=1)
    ...
    if avg_approximation == AverageApproximationFunction.RATIO:
        ...
        average_result = torch.sum(numeric_values_masked *
        ...
        average_result = torch.sum(numeric_values_masked *
                                   scaled_probability_per_cell * multiplier,
                                   dim=1)
    else:
        raise ValueError(
...
def huber_loss(input, target, delta: float = 1.0):
    ...
    return torch.where(errors < delta, 0.5 * errors**2,
                       errors * delta - (0.5 * delta**2))
...
def _calculate_regression_loss(
    ...
    # float32 (batch_size,)
    answer_masked = torch.where(torch.isnan(answer), torch.zeros_like(answer),
                                answer)
    ...
    if config.answer_loss_cutoff is None:
        large_answer_loss_mask = torch.ones_like(per_example_answer_loss,
                                                 dtype=torch.float32)
