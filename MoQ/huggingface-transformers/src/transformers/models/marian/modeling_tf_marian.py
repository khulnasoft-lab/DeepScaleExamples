...
class TFMarianDecoder(tf.keras.layers.Layer):
    ...
    def call(
        ...
        **kwargs,
        ...
        hidden_states = self.dropout(hidden_states + positions,
                                     training=inputs["training"])
    ...
    def adjust_logits_during_generation(
        ...
        **kwargs,
        ...
        if cur_len == 1 and forced_bos_token_id is not None:
            vocab_range = tf.constant(range(self.config.vocab_size))
            return tf.where(vocab_range != forced_bos_token_id, LARGE_NEGATIVE,
                            logits)
        elif cur_len == max_length - 1 and forced_eos_token_id is not None:
            vocab_range = tf.constant(range(self.config.vocab_size))
            return tf.where(vocab_range != forced_eos_token_id, LARGE_NEGATIVE,
                            logits)
        else:
            return logits
