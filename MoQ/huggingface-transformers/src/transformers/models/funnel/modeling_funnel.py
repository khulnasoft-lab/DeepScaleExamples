...
class FunnelAttentionStructure(nn.Module):
    ...
    def init_attention_inputs(self,
        ...
        cls_mask = (F.pad(inputs_embeds.new_ones([seq_len - 1, seq_len - 1]),
                          (1, 0, 1, 0)) if self.config.separate_cls else None)
    ...
    def stride_pool_pos(self, pos_id, block_index):
        ...
        if self.config.separate_cls:
            ...
            pooled_pos_id = pos_id[
                1:-1] if self.config.truncate_seq else pos_id[1:]
    ...
    def stride_pool(self, tensor, axis):
        ...
        axis_slice = (slice(None, -1, 2) if self.config.separate_cls
                      and self.config.truncate_seq else slice(None, None, 2))
    ...
    def pool_tensor(self, tensor, mode="mean", stride=2):
        ...
        if mode == "mean":
            tensor = F.avg_pool2d(tensor,
                                  stride,
                                  stride=stride,
                                  ceil_mode=True)
        elif mode == "max":
            tensor = F.max_pool2d(tensor,
                                  stride,
                                  stride=stride,
                                  ceil_mode=True)
        elif mode == "min":
...
def _relative_shift_gather(positional_attn, context_len, shift):
    ...
    positional_attn = torch.reshape(positional_attn,
...
class FunnelRelMultiheadAttention(nn.Module):
    ...
    def relative_positional_attention(self,
        ...
        if self.config.attention_type == "factorized":
            ...
            positional_attn = torch.einsum("binh,tnh->bnit", q_head + v,
                                           r_head)
            # Shape batch_size x n_head x seq_len x context_len
    ...
    def relative_token_type_attention(self,
        ...
        diff_token_type, same_token_type = torch.split(token_type_bias,
                                                       1,
                                                       dim=-1)
        # Shape batch_size x n_head x seq_len x context_len
    ...
    def forward(self,
        ...
        v_head = self.v_head(value).view(batch_size, context_len, n_head,
                                         d_head)
        ...
        content_score = torch.einsum("bind,bjnd->bnij", q_head + r_w_bias,
                                     k_head)
...
class FunnelEncoder(nn.Module):
    ...
    def forward(
        ...
        for block_index, block in enumerate(self.blocks):
            pooling_flag = hidden.size(1) > (2 if self.config.separate_cls else
                                             1)
            ...
            for layer_index, layer in enumerate(block):
                for repeat_index in range(
                    ...
                    if do_pooling:
                        query = pooled_hidden
                        key = value = (hidden if self.config.pool_q_only else
                                       pooled_hidden)
                    else:
...
class FunnelPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if classname.find("Linear") != -1:
            ...
            std = (1.0 if self.config.initializer_std is None else
...
class FunnelForPreTraining(FunnelPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(
        FUNNEL_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @replace_return_docstrings(output_type=FunnelForPreTrainingOutput,
                               config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids=None,
