...
class LEDLearnedPositionalEmbedding(nn.Embedding):
    ...
    def __init__(self, num_embeddings: int, embedding_dim: int,
        ...
        assert (
            padding_idx
            is not None), "`padding_idx` should not be None, but of type int"
...
class LEDEncoderAttention(nn.Module):
    def __init__(self, config, layer_id):
        super().__init__()
        self.longformer_self_attn = LEDEncoderSelfAttention(config,
                                                            layer_id=layer_id)
...
class LEDEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        if torch.isinf(hidden_states).any() or torch.isnan(
                hidden_states).any():
...
class LEDEncoder(LEDPreTrainedModel):
    ...
    def forward(
        ...
        if attention_mask is None:
            attention_mask = torch.ones(inputs_embeds.size()[:-1],
        ...
        # If the user passed a tuple for encoder_outputs, we wrap it in a LEDEncoderBaseModelOutput when return_dict=False
            ...
            encoder_outputs = LEDEncoderBaseModelOutput(
                ...
                global_attentions=(encoder_outputs[3]
                                   if len(encoder_outputs) > 3 else None),
            )
