...
from .integrations import (  # isort: split
    default_hp_search_backend, get_reporting_integration_callbacks, hp_params,
    is_fairscale_available, is_optuna_available, is_ray_tune_available,
    run_hp_search_optuna, run_hp_search_ray, init_deepscale,
)
...
class Trainer:
    ...
    Important attributes:
        ...
        - **model_wrapped** -- Always points to the most external model in case one or more other modules wrap the
          original model. This is the model that should be used for the forward pass. For example, under ``DeepScale``,
          the inner model is wrapped in ``DeepScale`` and then again in ``torch.nn.DistributedDataParallel``. If the
          inner model hasn't been wrapped, then ``self.model_wrapped`` is the same as ``self.model``.
        - **is_model_parallel** -- Whether or not a model has been switched to a model parallel mode (different from
          data parallelism, this means some of the model layers are split on different GPUs).
        - **place_model_on_device** -- Whether or not to automatically place the model on the device - it will be set
          to :obj:`False` if model parallel or deepscale is used, or if the default
    ...
    def __init__(
        ...
        if (hasattr(model, "is_parallelizable") and model.is_parallelizable
        ...
        if (self.is_model_parallel or (args.deepscale and args.do_train)
        ...
        # 2. fp16-enabled DeepScale loads the model in half the size and it doesn't need .to() anyway,
        #    and we only use deepscale for training at the moment
        ...
        if model_init is not None and (self.optimizer is not None
        ...
        if is_torch_tpu_available() and isinstance(self.model,
                                                   PreTrainedModel):
        ...
        if is_datasets_available():
            if isinstance(train_dataset, datasets.Dataset):
                self._remove_unused_columns(self.train_dataset,
                                            description="training")
            if isinstance(eval_dataset, datasets.Dataset):
                self._remove_unused_columns(self.eval_dataset,
        ...
        if args.sharded_ddp:
            if args.deepscale:
                raise ValueError(
                    "Using --sharded_ddp together with --deepscale is not possible, deactivate one of those flags."
                )
        ...
        if args.fp16 and not args.deepscale:  # deepscale manages its own fp16
        ...
        default_label_names = (["start_positions", "end_positions"] if type(
            self.model) in MODEL_FOR_QUESTION_ANSWERING_MAPPING.values() else
                               ["labels"])
        self.label_names = (default_label_names
                            if self.args.label_names is None else
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.sampler.Sampler]:
        ...
        elif (self.args.parallel_mode == ParallelMode.DISTRIBUTED or
    ...
    def get_eval_dataloader(self,
        ...
        if eval_dataset is None and self.eval_dataset is None:
            ...
            raise ValueError("eval_dataset must implement __len__")
        elif is_datasets_available() and isinstance(eval_dataset,
                                                    datasets.Dataset):
    ...
    def get_test_dataloader(self, test_dataset: Dataset) -> DataLoader:
        ...
        elif is_datasets_available() and isinstance(test_dataset,
                                                    datasets.Dataset):
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        if self.lr_scheduler is None:
            warmup_steps = (self.args.warmup_steps
                            if self.args.warmup_steps > 0 else math.ceil(
    ...
    def _hp_search_setup(self, trial: Union["optuna.Trial", Dict[str, Any]]):
        ...
        params = (self.hp_space(trial) if self.hp_search_backend
                  == HPSearchBackend.OPTUNA else trial)
    ...
    def _tune_save_checkpoint(self):
        ...
        with tune.checkpoint_dir(
                step=self.state.global_step) as checkpoint_dir:
    ...
    def train(
        ...
        **kwargs,
        ...
        if self.args.deepscale:
            model, optimizer, lr_scheduler = init_deepscale(
                self, num_training_steps=max_steps)
            self.model = model.module
            self.model_wrapped = model  # will get further wrapped in DDP
            self.deepscale = model  # DeepScaleEngine object
        ...
        # self.model_wrapped is DDP(Transformers Model), Deepscale(Transformers Model), etc.
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if (self.args.max_grad_norm is not None
                            ...
                            and not self.deepscale):
                        # deepscale does its own clipping
        ...
        if (self.args.load_best_model_at_end
            ...
            if self.deepscale:
                self.deepscale.load_checkpoint(
                    self.state.best_model_checkpoint,
        ...
        if self.deepscale:
            ...
            # to restore normal behavior outside of train replay the place_model_on_device logic w/o deepscale
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        # In all cases, including ddp/dp/deepscale, self.model is always a reference to the model we
        # want to save.
        assert (_model_unwrap(model) is self.model
        ...
        self.save_model(output_dir)
        if self.deepscale:
            self.deepscale.save_checkpoint(output_dir)
        ...
        elif self.is_world_process_zero() and not self.deepscale:
            # deepscale.save_checkpoint above saves model/optim/sched
    ...
    def _load_optimizer_and_scheduler(self, checkpoint):
        ...
        if os.path.isfile(os.path.join(
            ...
            if is_torch_tpu_available():
                ...
                xm.send_cpu_data_to_device(lr_scheduler_state,
                                           self.args.device)
        ...
        if self.deepscale:
            # Not sure how to check if there is a saved deepscale checkpoint, but since it just return None if it fails to find a deepscale checkpoint this is sort of a check-n-load function
            self.deepscale.load_checkpoint(checkpoint,
    ...
    def hyperparameter_search(
        ...
        hp_space: Optional[Callable[["optuna.Trial"], Dict[str,
                                                           float]]] = None,
        compute_objective: Optional[Callable[[Dict[str, float]],
                                             float]] = None,
        n_trials: int = 20,
        ...
        **kwargs,
        ...
        self.hp_space = default_hp_space[
            backend] if hp_space is None else hp_space
        self.hp_name = hp_name
        self.compute_objective = (default_compute_objective
                                  if compute_objective is None else
                                  compute_objective)
    ...
    def evaluate(
        ...
        output = self.prediction_loop(
            ...
            if self.compute_metrics is None else None,
    ...
    def prediction_loop(
        ...
        if self.args.deepscale and not self.args.do_train:
            # no harm, but flagging to the user that deepscale config is ignored for eval
            # flagging only for when --do_train wasn't passed as only then it's redundant
            logger.info(
                "Detected the deepscale argument but it will not be used for evaluation"
            )
        ...
        if not prediction_loss_only:
            preds_gatherer = DistributedTensorGatherer(world_size,
                                                       num_examples)
            labels_gatherer = DistributedTensorGatherer(
                world_size, num_examples)
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if (self.args.eval_accumulation_steps is not None
                ...
                if not prediction_loss_only:
                    ...
                    labels_gatherer.add_arrays(
                        self._gather_and_numpify(labels_host,
                                                 "eval_label_ids"))
