...
class NotebookProgressBar:
    ...
    def update(self,
        ...
        if self.last_value is None:
            ...
            return
        elif (force_update or self.first_calls > 0
              or value >= min(self.last_value + self.wait_for, self.total)):
            if self.first_calls > 0:
                self.first_calls -= 1
            current_time = time.time()
            self.elapsed_time = current_time - self.start_time
            self.average_time_per_item = self.elapsed_time / (value -
                                                              self.start_value)
            if value >= self.total:
                value = self.total
    ...
    def update_bar(self, value, comment=None):
        ...
        self.label += ("]" if self.comment is None or len(self.comment) == 0
                       else f", {self.comment}]")
        self.display()
    ...
    def display(self):
        ...
        if self.output is None:
            self.output = disp.display(disp.HTML(self.html_code),
                                       display_id=True)
        else:
...
class NotebookProgressCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        epoch = (int(state.epoch)
                 if int(state.epoch) == state.epoch else f"{state.epoch:.2f}")
