    ...
    def generate(
        ...
        if input_ids is not None:
            batch_size = shape_list(input_ids)[
                0]  # overridden by the input batch_size
        else:
            batch_size = 1

        assert (isinstance(max_length, int) and max_length > 0
                ), "`max_length` should be a strictly positive integer."
        assert (isinstance(min_length, int) and
                min_length >= 0), "`min_length` should be a positive integer."
        ...
        if ((attention_mask is None) and (pad_token_id is not None)
            ...
            attention_mask = tf.cast(tf.math.not_equal(input_ids,
                                                       pad_token_id),
                                     dtype=tf.int32)
        ...
        if self.config.is_encoder_decoder:
            ...
            encoder_outputs = (tf.gather(encoder_outputs[0],
                                         expanded_batch_idxs,
                                         axis=0), )
        else:
        ...
        while cur_len < max_length:
            ...
            if no_repeat_ngram_size > 0:
                ...
                next_token_logits = set_tensor_by_indices_to_value(
                    next_token_logits,
                    tf.convert_to_tensor(banned_tokens_indices_mask,
                                         dtype=tf.bool),
                    -float("inf"),
                )
            ...
            if do_sample:
                ...
                next_token = tf.squeeze(
                    tf.random.categorical(next_token_logits,
                                          dtype=tf.int32,
                                          num_samples=1),
                    axis=1,
                )
            ...
            if self.config.is_encoder_decoder is False:
                attention_mask = tf.concat(
                    [
                        attention_mask,
                        tf.ones((shape_list(attention_mask)[0], 1),
                                dtype=tf.int32),
                    ],
                    axis=-1,
                )
    ...
    def _generate_beam_search(
        ...
        while cur_len < max_length:
            ...
            scores = tf.nn.log_softmax(
                next_token_logits,
                axis=-1)  # (batch_size * num_beams, vocab_size)
            ...
            if do_sample:
                ...
                _scores = tf.reshape(_scores,
                                     (batch_size, num_beams * vocab_size))

                next_tokens = sample_without_replacement(
                    _scores,
                    num_samples=2 * num_beams)  # (batch_size, 2 * num_beams)
                # Compute next scores
                ...
                next_scores, next_tokens = tf.math.top_k(next_scores,
                                                         k=2 * num_beams,
                                                         sorted=True)
        ...
        if tf.reduce_min(sent_lengths).numpy() != tf.reduce_max(
                sent_lengths).numpy():
                ...
                if sent_lengths[i] == sent_max_len:
                    ...
                    if sent_lengths[i] < max_length:
                        decoded_slice = tf.where(
                            ...
                            eos_token_id * tf.ones(
                                (sent_max_len, ), dtype=tf.int32),
                            decoded_slice,
                        )
                # add to list
...
def calc_banned_ngram_tokens(prev_input_ids, num_hypos, no_repeat_ngram_size,
                             cur_len):
    ...
    def _get_generated_ngrams(hypo_idx):
        ...
        ngram_idx = tuple(prev_input_ids[hypo_idx,
                                         start_idx:cur_len].numpy().tolist())
...
def tf_top_k_top_p_filtering(logits,
                             top_k=0,
                             top_p=1.0,
    ...
    if top_p < 1.0:
        ...
        sorted_indices_to_remove = tf.roll(sorted_indices_to_remove,
                                           1,
                                           axis=-1)
        sorted_indices_to_remove = tf.concat(
            [
