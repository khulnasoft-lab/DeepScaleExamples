...
def convert_tf_weight_name_to_pt_weight_name(tf_name,
    ...
    transpose = bool(tf_name[-1] == "kernel" or "emb_projs" in tf_name
                     or "out_projs" in tf_name)

    # Convert standard TF2.0 names in PyTorch names
    if tf_name[-1] == "kernel" or tf_name[-1] == "embeddings" or tf_name[
            -1] == "gamma":
        tf_name[-1] = "weight"
    if tf_name[-1] == "beta":
        tf_name[-1] = "bias"
...
def load_pytorch_checkpoint_in_tf2_model(tf_model,
...
def load_pytorch_model_in_tf2_model(tf_model,
                                    pt_model,
                                    tf_inputs=None,
                                    allow_missing_keys=False):
    """Load pytorch checkpoints in a TF 2.0 model"""
...
def load_tf2_checkpoint_in_pytorch_model(pt_model,
                                         tf_checkpoint_path,
                                         tf_inputs=None,
                                         allow_missing_keys=False):
    """
    ...
    logger.info(
        "Loading TensorFlow weights from {}".format(tf_checkpoint_path))
...
def load_tf2_model_in_pytorch_model(pt_model,
                                    tf_model,
                                    allow_missing_keys=False):
...
def load_tf2_weights_in_pytorch_model(pt_model,
                                      tf_weights,
                                      allow_missing_keys=False):
    ...
    logger.info("Weights or buffers not loaded from TF 2.0 model: {}".format(
        all_tf_weights))
