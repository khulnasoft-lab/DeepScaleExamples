...
from .integrations import (  # isort: split
    is_comet_available, is_wandb_available,
)
...
class TFTrainer:
    ...
    def __init__(
        ...
        if is_wandb_available():
            self.setup_wandb()
        elif os.getenv("WANDB_DISABLED",
                       "").upper() not in ENV_VARS_TRUE_VALUES:
            logger.info(
    ...
    def get_train_tfdataset(self) -> tf.data.Dataset:
        ...
        ds = (self.train_dataset.repeat().shuffle(
            self.num_train_examples, seed=self.args.seed).batch(
    ...
    def get_eval_tfdataset(
            self,
            eval_dataset: Optional[tf.data.Dataset] = None) -> tf.data.Dataset:
        """
    ...
    def setup_comet(self):
        ...
        if experiment is not None:
            ...
            experiment._log_parameters(self.args,
                                       prefix="args/",
    ...
    def evaluate(
        ...
        output = self.prediction_loop(eval_ds,
                                      steps,
                                      num_examples,
    ...
    def train(self) -> None:
        ...
        num_update_steps_per_epoch = (self.num_train_examples /
                                      self.total_train_batch_size)
        ...
        if self.args.gradient_accumulation_steps == 1:
            ...
            for _ in tf.range(self.args.gradient_accumulation_steps):
                ...
                if tf.is_tensor(labels):
                    ...
                    reduced_labels = {
                        k: lbl[:self.args.train_batch_size //
                               self.args.n_replicas]
    ...
    def run_model(self, features, labels, training):
        ...
        if self.args.past_index >= 0 and getattr(self, "_past",
                                                 None) is not None:
    ...
    def predict(self, test_dataset: tf.data.Dataset) -> PredictionOutput:
        ...
        return self.prediction_loop(test_ds,
                                    steps,
                                    num_examples,
