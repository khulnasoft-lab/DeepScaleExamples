...
def generate_samples_unconditional(model):
    ...
    context_tokens = [[tokenizer.eod] for _ in range(args.batch_size)]
    ctr = 0
    while True:
...
def sample_sequence_batch(
    ...
    with torch.no_grad():
        ...
        while context_length <= (maxlen):
            ...
            for p in prev:
                print_logits.append(
                    [logits[i, p].item() for i in range(batch_size)])
            started = context_lengths <= context_length
            tokens[:,
                   context_length] = switch(tokens[:, context_length].view(-1),
                                            prev, started)
            context_length += 1
            counter += 1
