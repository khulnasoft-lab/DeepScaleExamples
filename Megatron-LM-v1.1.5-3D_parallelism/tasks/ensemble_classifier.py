            ...
            for dataset in data:
                ...
                if name not in all_predictions:
                    ...
                    if args.labels is None:
                        args.labels = [
                            i for i in range(all_predictions[name].shape[1])
                        ]
                    if args.eval:
...
def get_threshold(all_predictions, all_labels, one_threshold=False):
    if one_threshold:
        ...
        all_labels = {
            "combined": np.concatenate(list(all_predictions.labels()))
        }
    out_thresh = []
...
def postprocess_predictions(all_predictions, all_labels, args):
    ...
    if args.threshold is not None:
        all_predictions = threshold_predictions(all_predictions,
                                                args.threshold)
...
def write_predictions(all_predictions, all_labels, all_uid, args):
    ...
    for dataset in all_predictions:
        ...
        outpath = os.path.join(
            args.outdir, dataset,
            os.path.splitext(args.prediction_name)[0] + ".tsv")
