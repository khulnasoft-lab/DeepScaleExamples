...
def biencoder_model_provider(
    ...
    assert (mpu.get_tensor_model_parallel_world_size() == 1
            ...
            == 1), "Model parallel size > 1 not supported for ICT"
...
class BiEncoderModel(MegatronModule):
    ...
    def state_dict_for_save_checkpoint(self,
        ...
        if self.biencoder_shared_query_context_model:
            state_dict_[
                self._model_key] = self.model.state_dict_for_save_checkpoint(
...
class PretrainedBertModel(MegatronModule):
    ...
    def state_dict_for_save_checkpoint(self,
        ...
        if self.biencoder_projection_dim > 0:
            state_dict_[
                self._projection_enc_key] = self.projection_enc.state_dict(
                    destination, prefix, keep_vars)
