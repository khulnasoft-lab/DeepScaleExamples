...
if __name__ == "__main__":
    ...
    if args.load_fingerprints is not None:
        for count_fp, fp_file_name in enumerate(args.load_fingerprints):
            print(
                "Loading fingerprints from pickle file {}".format(
                    fp_file_name),
                flush=True,
            )
            ...
            if count_fp == 0:
                ...
                for url in local_lshcache.fingerprints.keys():
                    ...
                    lshcache.add_fingerprint(local_lshcache.fingerprints[url],
                                             url)
            fp.close()
    ...
    if args.save_fingerprints is not None:
        print(
            "Saving fingerprints to pickle file {}".format(
