...
class ORQAEvaluator(object):
    def __init__(self):
        ...
        model = get_model(lambda: biencoder_model_provider(
            only_query_model=only_query_model,
            biencoder_shared_query_context_model=args.
    ...
    def evaluate(self, qa_data, split):
        args = get_args()
        query_tensor, reference_list = self.generate_query_vectors(
            qa_data, split)
        ...
        torch.distributed.broadcast(distance,
                                    src=device_start_rank,
                                    group=group)
        torch.distributed.broadcast(topkindex,
                                    src=device_start_rank,
                                    group=group)

        distance = torch.split(distance, len(query_tensor), dim=0)[local_rank]
        topkindex = torch.split(topkindex, len(query_tensor),
                                dim=0)[local_rank]
