...
def forward_step(data_iterator, model, args, timers):
    ...
    tokens, types, next_sentence, loss_mask, lm_labels, padding_mask = get_batch(
...
def train_step(data_iterator, model, optimizer, lr_scheduler, args, timers):
    ...
    lm_loss, nsp_loss = forward_step(data_iterator, model, args, timers)
    timers("forward").stop()

    # Calculate gradients, reduce across processes, and clip.
    timers("backward").start()
    lm_loss_reduced, nsp_loss_reduced = backward_step(optimizer, model,
                                                      lm_loss, nsp_loss, args)
...
def evaluate(data_iterator, model, args, timers, verbose=False):
    ...
    with torch.no_grad():
        ...
        while iteration < args.eval_iters:
            ...
            # Forward evaluation.
            lm_loss, nsp_loss = forward_step(data_iterator, model, args,
                                             timers)
            # Reduce across processes.
...
def main():
    ...
    (
        ...
        args.tokenizer_num_type_tokens,
