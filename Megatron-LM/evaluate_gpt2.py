...
def get_masks_and_position_ids(data, eod_token, reset_position_ids,
...
def get_batch(data_iterator, args, timers):
    ...
    attention_mask, loss_mask, position_ids = get_masks_and_position_ids(
        tokens, args.eod_token, args.reset_position_ids,
...
def evaluate(data_loader, model, args, timers, num_iterations=None):
    """Evaluation."""
...
def evaluate_and_print_results(prefix,
    ...
    if not args.cloze_eval:
        ...
        num_correct = evaluate(data_iterator, model, args, timers,
                               num_iterations)
...
def get_eval_data(args):
    ...
    if mpu.get_model_parallel_rank() == 0:
        ...
        valid_data = valid_data[0] if isinstance(valid_data,
                                                 list) else valid_data
        ...
        if not args.cloze_eval:
            ...
            string = "Original Tokens: %d, Detokenized tokens: %d" % (
        ...
        token_counts = torch.cuda.LongTensor([
            after, eod_token, num_examples, num_original_tokens,
            num_tokenized_tokens
        ])
    else:
...
def main():
    ...
    if args.eval_hf:
        ...
        if args.num_layers == 24:
            ...
            hfmodel = HFGPT2Model.from_pretrained(model_path,
                                                  cache_dir="gpt2_weights",
                                                  from_tf=True).cuda()
            ...
            model = GPT2LMHeadModel.from_pretrained(
                "gpt2", cache_dir="gpt2_weights").cuda()
    else:
        if args.load_openai:
    ...
    evaluate_and_print_results(prefix, eval_data, model, args, timers)
