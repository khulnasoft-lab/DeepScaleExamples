...
# Force deepscale to run with only local node
NUM_NODES=1
HOSTFILE=/dev/null
...
JOB_NAME="deepscale_${NGPU}GPUs_${EFFECTIVE_BATCH_SIZE}batch_size"
config_json=deepscale_bsz24_config.json
run_cmd="deepscale --num_nodes ${NUM_NODES} --num_gpus ${NGPU_PER_NODE} \
       --master_port=${MASTER_PORT} \
       --hostfile ${HOSTFILE} \
       nvidia_run_squad_deepscale.py \
