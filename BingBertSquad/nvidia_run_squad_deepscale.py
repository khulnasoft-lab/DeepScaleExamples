...
def main():
    ...
    if args.ckpt_type == "DS":
        ...
        # Use the original bert config if want to load from non-DeepScale checkpoint.
        if args.origin_bert_config_file is None:
            raise ValueError(
                "--origin_bert_config_file is required for loading non-DeepScale checkpoint."
            )
    ...
    if args.model_file is not "0":
        ...
        if args.ckpt_type == "DS":
            ...
            from convert_bert_ckpt_to_deepscale import convert_ckpt_to_deepscale

            convert_ckpt_to_deepscale(
                model,
                args.ckpt_type,
                args.model_file,
                vocab_diff,
                args.deepscale_transformer_kernel,
            )
    ...
    logger.info("propagate deepscale-config settings to client settings")
                ...
                if (step + 1) % (
                    ...
                    print(
                        ...
                        .format(
                            ...
                            ave_rounds,
                            bs_size * ave_rounds *
                            torch.distributed.get_world_size() / all_step_time,
                        ),
                        flush=True,
                    )
                    all_step_time = 0.0
