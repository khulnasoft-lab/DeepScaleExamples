...
def make_dataset(path,
    ...
    def get_dataset_from_path(path_):
        if lazy:
            ...
            if torch.distributed.get_rank() == 0 and not exists_lazy(
                    path_, data_type="data"):
                # create cached version of dataset for lazy loading if it doesn't exist
                text = get_dataset(
                    name if named_corpora else path_,
                    text_key=text_key,
                    label_key=label_key,
    ...
    if tokenizer is None:
        tokenizer = make_tokenizer(tokenizer_type, ds, tokenizer_model_path,
    ...
    if should_split(split):
        ...
        if "bert" in ds_type.lower():
            presplit_sentences = (kwargs["presplit_sentences"]
                                  if "presplit_sentences" in kwargs else False)
            ...
            ds = [
                GPT2Dataset(d, max_seq_len=seq_length)
                if d is not None else None for d in ds
            ]
    else:
        if "bert" in ds_type.lower():
