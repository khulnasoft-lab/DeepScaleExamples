...
class ParallelMLP(MegatronModule):
    ...
    def __init__(self, init_method, output_layer_init_method):
        ...
        if not args.memory_centric_tiled_linear:
            ...
            self.dense_4h_to_h = deepscale.zero.TiledLinearReturnBias(
...
class ParallelSelfAttention(MegatronModule):
    ...
    def __init__(self, attention_mask_func, init_method,
        ...
        if not args.memory_centric_tiled_linear:
            ...
            self.query_key_value = deepscale.zero.TiledLinearReturnBias(
                in_features=args.hidden_size,
        ...
        if not args.memory_centric_tiled_linear:
            ...
            self.dense = deepscale.zero.TiledLinearReturnBias(
                in_features=args.hidden_size,
                out_features=args.hidden_size,
                linear_cls=mpu.RowParallelLinear,
                input_is_parallel=True,
        ...
        if deepscale.checkpointing.is_configured():
            ...
            get_cuda_rng_tracker = deepscale.checkpointing.get_cuda_rng_tracker
    ...
    def forward(self,
        ...
        if checkpoint_version is not None:
            if checkpoint_version == 0:
                ...
                mixed_x_layer = self._transpose_last_dim(
                    mixed_x_layer, 3, True)
            elif checkpoint_version == 1.0:
                # [s, b, (np * hn * 3)] --> [s, b, (np * 3 * hn)]
                mixed_x_layer = self._transpose_last_dim(
                    mixed_x_layer, 3, False)
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def forward(self,
        ...
        if self.scattered_attn_output:
            attention_output = mpu.scatter_to_model_parallel_region(
                attention_output)
            attention_bias = mpu.scatter_to_model_parallel_region(
                attention_bias)
    ...
    def forward(self,
        ...
        if get_key_value:
            raise NotImplementedError(
                "get_key_value param is not yet supported with split-transformers"
            )
            attention_output, presents = attention_output
...
class ParallelTransformer(MegatronModule):
    ...
    def __init__(self, attention_mask_func, init_method,
        ...
        def build_layer_part1(layer_number):
            return ParallelTransformerLayerPart1(attention_mask_func,
                                                 init_method,
                                                 output_layer_init_method,
                                                 layer_number)

        def build_layer_part2(layer_number):
            return ParallelTransformerLayerPart2(attention_mask_func,
                                                 init_method,
                                                 output_layer_init_method,
                                                 layer_number)
