    ...
    for _, batch_index in enumerate(tqdm(dataset_iterator, smoothing=1)):
        try:
            ...
            if model.network.is_gradient_accumulation_boundary():
                ...
                # Call DeepScale engine step on micro steps
                model.network.step()
            ...
            if any(nd in n for nd in need_mask):
                ...
                for position in range(args.max_seq_length):
                    for col in range(p.size()[1]):
                        mask[position][col] += 1
                if deepscale_config["optimizer"]["type"] == "OneBitAdam":
...
def prepare_model_optimizer(args):
    ...
    # DeepScale initializer handles FP16, distributed, optimizer automatically.
    model.network, optimizer, _, _ = deepscale.initialize(
        args=args,
        model=model.network,
        model_parameters=optimizer_grouped_parameters)

    # Overwrite application configs with DeepScale config
